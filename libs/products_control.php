  ...
  //====GetProducts==========================================================
   if($key == 'All'){
      ...
      $sql = "SELECT 	`tovar_inet_id_parent`,
               ...
               `tovar_inet_id` > 0 $brand_filter $attr_str
                GROUP BY tovar_name_1
               ORDER BY  T.tovar_id DESC, T.sort ASC, items DESC, price_1 ASC, `tovar_name_1` ASC
             LIMIT $start, $step
             ";
   }
//echo $sql;
       ...
       while($tmp = $getName->fetch_assoc()){
          ...
          //Разбиваем атрикл на тело и размер
           ...
           //Считаем количество артиклей в подкатегориях
            ...
            $alias = $Alias->getProductAlias($tmp['tovar_id']);
             $products[$artkl]['name'] = $tmp['tovar_name'];
             $products[$artkl]['id'] = $tmp['tovar_id'];
             $products[$artkl]['alias'] = HOST_URL.'/'.$alias;
             ...
             $products[$artkl]['size'][$size]['price'] = $tmp['price_1'];//$Product->getProductPrice($tmp['tovar_id']);
             $products[$artkl]['size'][$size]['curr'] = $tmp['curr2'];
             $products[$artkl]['size'][$size]['items'] = $tmp['items'];//$Product->getProductOnWare($tmp['tovar_id']);
      
             //Будем брать описание если оно не пустое.
             $tmp_memo = $Product->getProductMemoShort($alias);
             if($tmp_memo != ''){
             ...
             if(isset($products[$artkl]['total'])){
                $products[$artkl]['total'] += $tmp['items'];//$Product->getProductOnWare($tmp['tovar_id']);
             }else{
                $products[$artkl]['total'] = $tmp['items'];//$Product->getProductOnWare($tmp['tovar_id']);
             }
             
             //Наименьшая цена на морду 
             $tmp_price = $tmp['price_1']; //$Product->getProductPrice($tmp['tovar_id']);
			  if($tmp_price > 0){
			  ...
			  }elseif(!isset($products[$artkl]['price'])){
				 $products[$artkl]['price'] = $tmp_price;
				 $products[$artkl]['curr'] = $tmp['curr2'];
			  }
			 }
