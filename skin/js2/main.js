...
$(function () {
    ...
    $('.hc-menu')
        .on('mouseenter', '.hc-menu__item', function () {
            var $this = $(this);
            if ($this.find('.hc-menu__sub').length) {
                $this.find('.hc-menu__link').addClass('hc-menu__link_hover');
                $this.find('.hc-menu__sub').stop(true, false).fadeIn(100);
            }
        })
        .on('mouseleave', '.hc-menu__item', function () {
            var $this = $(this);
            if ($this.find('.hc-menu__sub').length) {
                $this.find('.hc-menu__link').removeClass('hc-menu__link_hover');
                $this.find('.hc-menu__sub').stop(true, false).fadeOut(100);
            }
        })
        .on('click', '.hc-menu__drop-search', function (e) {
            e.preventDefault();
            $(this).toggleClass('hc-menu__drop-search_active');
            $('#medium-drop-search').stop(true, false).fadeToggle(100);
        });

    $('.filters')
        .on('click', '.filters__more-link', function () {
            var $this = $(this);
            $this.closest('.filters__more').toggleClass('filters__more_active');
            $this.closest('.filters__group').find('.filters__accordion').toggleClass('filters__accordion_active');
        })
        .on('click', '.filters__title', function () {
            if (Foundation.utils.is_small_only()) {
                var $this = $(this);
                $this.toggleClass('filters__title_active');
                $this.closest('.filters').find('.filters__content').stop(true, false).slideToggle(100);
            }
        });
    ...
    $(window)
       .on('resize', Foundation.utils.throttle(function () {
           if (!Foundation.utils.is_small_only()) {
               $('.filters__title').each(function (e) {
                   $(this).closest('.filters').find('.filters__content').stop(true, false).show();
               });
           } else {
               $('.filters__title').each(function (e) {
                   $(this).closest('.filters').find('.filters__content').stop(true, false).hide();
               });
           }
       }, 100));
    ...
    $('.product__size')
        .on('click', function (e) {
            var $this = $(this);
            $this.closest('.product__sizes').find('.product__size_active').removeClass('product__size_active');
    ...
    $('.js-tooltip-img')
        .tooltipster({
            ...
            functionInit: function (origin, content) {
                return '<img src="' + origin.data('img') + '" alt="">'
            }
        });
});