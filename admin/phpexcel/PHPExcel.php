...
class 
{
    ...
    /**
     * Create a new  with one Worksheet
     */
    public function __construct()
    {
		$this->_uniqueID = uniqid();
		$this->_calculationEngine	= _Calculation::getInstance($this);

        // Initialise worksheet collection and add one worksheet
        $this->_workSheetCollection = array();
        $this->_workSheetCollection[] = new _Worksheet($this);
        $this->_activeSheetIndex = 0;

        // Create document properties
        $this->_properties = new _DocumentProperties();

        // Create document security
        $this->_security = new _DocumentSecurity();

        // Set named ranges
        $this->_namedRanges = array();

        // Create the cellXf supervisor
        $this->_cellXfSupervisor = new _Style(true);
        $this->_cellXfSupervisor->bindParent($this);

        // Create the default style
        $this->addCellXf(new _Style);
        $this->addCellStyleXf(new _Style);
    }
    ...
    public function __destruct() {
        _Calculation::unsetInstance($this);
        $this->disconnectWorksheets();
    }    //    function __destruct()

    /**
     * Disconnect all worksheets from this  workbook object,
     *    typically so that the  object can be unset
     *
     */
     ...
     * Set properties
     *
     * @param _DocumentProperties    $pValue
     */
    public function setProperties(_DocumentProperties $pValue)
    {
     ...
     * Set security
     *
     * @param _DocumentSecurity    $pValue
     */
    public function setSecurity(_DocumentSecurity $pValue)
    {
     ...
     * @throws _Exception
     */
    public function addSheet(_Worksheet $pSheet, $iSheetIndex = NULL)
    {
     ...
     * @throws _Exception
     */
    public function getIndex(_Worksheet $pSheet)
    {
        ...
        throw new _Exception("Sheet does not exist.");
    }
    ...
    {
        if (($worksheet = $this->getSheetByName($pValue)) instanceof _Worksheet) {
            $this->setActiveSheetIndex($this->getIndex($worksheet));
            return $worksheet;
        }

        throw new _Exception('Workbook does not contain sheet:' . $pValue);
    }
     ...
     * Add external sheet
     *
     * @param  _Worksheet $pSheet External sheet to add
     * @param  int|null $iSheetIndex Index where sheet should go (0,1,..., or null for last)
     * @throws _Exception
     * @return _Worksheet
     */
    public function addExternalSheet(_Worksheet $pSheet, $iSheetIndex = null) {
        if ($this->sheetNameExists($pSheet->getTitle())) {
            throw new _Exception("Workbook already contains a worksheet named '{$pSheet->getTitle()}'. Rename the external sheet first.");
        }
     ...
     * Add named range
     *
     * @param  _NamedRange $namedRange
     * @return 
     */
    public function addNamedRange(_NamedRange $namedRange) {
     ...
     * @return _NamedRange|null
     */
    public function getNamedRange($namedRange, _Worksheet $pSheet = null) {
        $returnValue = null;
     ...
     * @return 
     */
    public function removeNamedRange($namedRange, _Worksheet $pSheet = null) {
        if ($pSheet === NULL) {
            if (isset($this->_namedRanges[$namedRange])) {
     ...
     *
     * @return _WorksheetIterator
     */
    public function getWorksheetIterator() {
        return new _WorksheetIterator($this);
    }
    ...
    public function getDefaultStyle()
    {
        ...
        throw new _Exception('No default style found for this workbook');
    }
     ...
     * @param _Style $style
     */
    public function addCellXf(_Style $style)
    {
    ...
    public function removeCellXfByIndex($pIndex = 0)
    {
        if ($pIndex > count($this->_cellXfCollection) - 1) {
            throw new _Exception("CellXf index is out of bounds.");
        } else {
     ...
     * @param _Style $pStyle
     */
    public function addCellStyleXf(_Style $pStyle)
    {
    ...
    public function removeCellStyleXfByIndex($pIndex = 0)
    {
        if ($pIndex > count($this->_cellStyleXfCollection) - 1) {
            throw new _Exception("CellStyleXf index is out of bounds.");
        } else {
    ...
    public function garbageCollect()
    {
        ...
        if (empty($this->_cellXfCollection)) {
            $this->_cellXfCollection[] = new _Style();
        }
