...
class _ReferenceHelper
{
	...
	public static function getInstance() {
		if (!isset(self::$_instance) || (self::$_instance === NULL)) {
			self::$_instance = new _ReferenceHelper();
		}
	...
	private static function cellAddressInDeleteRange($cellAddress, $beforeRow, $pNumRows, $beforeColumnIndex, $pNumCols) {
		list($cellColumn, $cellRow) = _Cell::coordinateFromString($cellAddress);
		$cellColumnIndex = _Cell::columnIndexFromString($cellColumn);
	 ...
	 *
	 * @param   _Worksheet  $pSheet             The worksheet that we're editing
	 ...
	 * @param   integer             $pNumRows           Number of rows to insert/delete (negative values indicate deletion)
	 */
	protected function _adjustPageBreaks(_Worksheet $pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
	{
		$aBreaks = $pSheet->getBreaks();
		($pNumCols > 0 || $pNumRows > 0) ?
			uksort($aBreaks, array('_ReferenceHelper','cellReverseSort')) :
			uksort($aBreaks, array('_ReferenceHelper','cellSort'));

		foreach ($aBreaks as $key => $value) {
			if (self::cellAddressInDeleteRange($key, $beforeRow, $pNumRows, $beforeColumnIndex, $pNumCols)) {
				//	If we're deleting, then clear any defined breaks that are within the range
				//		of rows/columns that we're deleting
				$pSheet->setBreak($key, _Worksheet::BREAK_NONE);
			} else {
				...
				if ($key != $newReference) {
					$pSheet->setBreak($newReference, $value)
					    ->setBreak($key, _Worksheet::BREAK_NONE);
				}
			}
		}
	}
	...
	protected function _adjustHyperlinks($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
	{
		...
		($pNumCols > 0 || $pNumRows > 0) ?
			uksort($aHyperlinkCollection, array('_ReferenceHelper','cellReverseSort')) :
			uksort($aHyperlinkCollection, array('_ReferenceHelper','cellSort'));
	...
	protected function _adjustDataValidations($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
	{
		...
		($pNumCols > 0 || $pNumRows > 0) ?
			uksort($aDataValidationCollection, array('_ReferenceHelper','cellReverseSort')) :
			uksort($aDataValidationCollection, array('_ReferenceHelper','cellSort'));
	...
	protected function _adjustProtectedCells($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
	{
		...
		($pNumCols > 0 || $pNumRows > 0) ?
			uksort($aProtectedCells, array('_ReferenceHelper','cellReverseSort')) :
			uksort($aProtectedCells, array('_ReferenceHelper','cellSort'));
	...
	protected function _adjustColumnDimensions($pSheet, $pBefore, $beforeColumnIndex, $pNumCols, $beforeRow, $pNumRows)
	{
		...
		if (!empty($aColumnDimensions)) {
			foreach ($aColumnDimensions as $objColumnDimension) {
				...
				list($newReference) = _Cell::coordinateFromString($newReference);
	 ...
	 * @throws  _Exception
	 */
	public function insertNewBefore($pBefore = 'A1', $pNumCols = 0, $pNumRows = 0, _Worksheet $pSheet = NULL)
	{
		...
		$beforeColumn	= 'A';
		$beforeRow		= 1;
		list($beforeColumn, $beforeRow) = _Cell::coordinateFromString($pBefore);
		$beforeColumnIndex = _Cell::columnIndexFromString($beforeColumn);
		...
		if ($pNumCols < 0 && $beforeColumnIndex - 2 + $pNumCols > 0) {
			for ($i = 1; $i <= $highestRow - 1; ++$i) {
				for ($j = $beforeColumnIndex - 1 + $pNumCols; $j <= $beforeColumnIndex - 2; ++$j) {
					$coordinate = _Cell::stringFromColumnIndex($j) . $i;
					$pSheet->removeConditionalStyles($coordinate);
					if ($pSheet->cellExists($coordinate)) {
						$pSheet->getCell($coordinate)->setValueExplicit('', _Cell_DataType::TYPE_NULL);
		...
		if ($pNumRows < 0 && $beforeRow - 1 + $pNumRows > 0) {
			for ($i = $beforeColumnIndex - 1; $i <= _Cell::columnIndexFromString($highestColumn) - 1; ++$i) {
				for ($j = $beforeRow + $pNumRows; $j <= $beforeRow - 1; ++$j) {
					$coordinate = _Cell::stringFromColumnIndex($i) . $j;
					$pSheet->removeConditionalStyles($coordinate);
					if ($pSheet->cellExists($coordinate)) {
						$pSheet->getCell($coordinate)->setValueExplicit('', _Cell_DataType::TYPE_NULL);
						$pSheet->getCell($coordinate)->setXfIndex(0);
		...
		while (($cellID = $remove ? array_shift($aCellCollection) : array_pop($aCellCollection))) {
			...
			$cellIndex = _Cell::columnIndexFromString($cell->getColumn());
			if ($cellIndex-1 + $pNumCols < 0) {
				continue;
			}

			// New coordinates
			$newCoordinates = _Cell::stringFromColumnIndex($cellIndex-1 + $pNumCols) . ($cell->getRow() + $pNumRows);
			...
			if (($cellIndex >= $beforeColumnIndex) &&
				...
				if ($cell->getDataType() == _Cell_DataType::TYPE_FORMULA) {
		...
		if ($pNumCols > 0 && $beforeColumnIndex - 2 > 0) {
			for ($i = $beforeRow; $i <= $highestRow - 1; ++$i) {
				...
				// Style
				$coordinate = _Cell::stringFromColumnIndex( $beforeColumnIndex - 2 ) . $i;
				if ($pSheet->cellExists($coordinate)) {
					...
					for ($j = $beforeColumnIndex - 1; $j <= $beforeColumnIndex - 2 + $pNumCols; ++$j) {
						...
						if ($conditionalStyles) {
							...
							$pSheet->setConditionalStyles(_Cell::stringFromColumnIndex($j) . $i, $cloned);
						}
					}
				}
		...
		if ($pNumRows > 0 && $beforeRow - 1 > 0) {
			for ($i = $beforeColumnIndex - 1; $i <= _Cell::columnIndexFromString($highestColumn) - 1; ++$i) {
				...
				// Style
				$coordinate = _Cell::stringFromColumnIndex($i) . ($beforeRow - 1);
				if ($pSheet->cellExists($coordinate)) {
					$xfIndex = $pSheet->getCell($coordinate)->getXfIndex();
					$conditionalStyles = $pSheet->conditionalStylesExists($coordinate) ?
						$pSheet->getConditionalStyles($coordinate) : false;
					for ($j = $beforeRow; $j <= $beforeRow - 1 + $pNumRows; ++$j) {
						$pSheet->getCell(_Cell::stringFromColumnIndex($i) . $j)->setXfIndex($xfIndex);
						if ($conditionalStyles) {
							$cloned = array();
							foreach ($conditionalStyles as $conditionalStyle) {
								$cloned[] = clone $conditionalStyle;
							}
							$pSheet->setConditionalStyles(_Cell::stringFromColumnIndex($i) . $j, $cloned);
						}
					}
				}
			}
		...
		if (!empty($autoFilterRange)) {
			if ($pNumCols != 0) {
				...
				if (count($autoFilterColumns) > 0) {
					...
					list($rangeStart,$rangeEnd) = _Cell::rangeBoundaries($autoFilterRange);
					if ($columnIndex <= $rangeEnd[0]) {
						if ($pNumCols < 0) {
							...
							for ($i = 1; $i <= $deleteCount; ++$i) {
								if (in_array(_Cell::stringFromColumnIndex($deleteColumn),$autoFilterColumns)) {
									$autoFilter->clearColumn(_Cell::stringFromColumnIndex($deleteColumn));
								}
								++$deleteColumn;
							}
						}
						...
						if ($pNumCols > 0) {
							...
							$startColID = _Cell::stringFromColumnIndex($startCol-1);
							$toColID = _Cell::stringFromColumnIndex($startCol+$pNumCols-1);
							$endColID = _Cell::stringFromColumnIndex($rangeEnd[0]);
							...
							do {
								$autoFilter->shiftColumn(_Cell::stringFromColumnIndex($endColRef-1),_Cell::stringFromColumnIndex($toColRef-1));
								--$endColRef;
								--$toColRef;
					...
					foreach($matches as $match) {
						...
						if ($match[3].':'.$match[4] !== $modified3.':'.$modified4) {
							if (($match[2] == '') || (trim($match[2],"'") == $sheetName)) {
								...
								$toString .= $modified3.':'.$modified4;
								//	Max worksheet size is 1,048,576 rows by 16,384 columns in Excel 2007, so our adjustments need to be at least one digit more
								$column = _Cell::columnIndexFromString(trim($match[3],'$')) + 100000;
								$row = 10000000;
								$cellIndex = $column.$row;
					...
					foreach($matches as $match) {
						...
						if ($match[3].$match[4] !== $modified3.$modified4) {
							if (($match[2] == '') || (trim($match[2],"'") == $sheetName)) {
								$toString = ($match[2] > '') ? $match[2].'!' : '';
								$toString .= $modified3.':'.$modified4;
								list($column,$row) = _Cell::coordinateFromString($match[3]);
								//	Max worksheet size is 1,048,576 rows by 16,384 columns in Excel 2007, so our adjustments need to be at least one digit more
								$column = _Cell::columnIndexFromString(trim($column,'$')) + 100000;
	 ...
	 *
	 * @param  $pPhpExcel	Object to update
	...
	public function updateNamedFormulas( $pPhpExcel, $oldName = '', $newName = '') {
		...
		foreach ($pPhpExcel->getWorksheetIterator() as $sheet) {
			foreach ($sheet->getCellCollection(false) as $cellID) {
				...
				if (($cell !== NULL) && ($cell->getDataType() == _Cell_DataType::TYPE_FORMULA)) {
	...
	private function _updateCellRange($pCellRange = 'A1:A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0) {
		if (strpos($pCellRange,':') !== false || strpos($pCellRange, ',') !== false) {
			// Update range
			$range = _Cell::splitRange($pCellRange);
			$ic = count($range);
			for ($i = 0; $i < $ic; ++$i) {
				$jc = count($range[$i]);
				for ($j = 0; $j < $jc; ++$j) {
					if (ctype_alpha($range[$i][$j])) {
						$r = _Cell::coordinateFromString($this->_updateSingleCellReference($range[$i][$j].'1', $pBefore, $pNumCols, $pNumRows));
						$range[$i][$j] = $r[0];
					} elseif(ctype_digit($range[$i][$j])) {
						$r = _Cell::coordinateFromString($this->_updateSingleCellReference('A'.$range[$i][$j], $pBefore, $pNumCols, $pNumRows));
						$range[$i][$j] = $r[1];
					} else {
			...
			return _Cell::buildRange($range);
		} else {
			throw new _Exception("Only cell ranges may be passed to this method.");
		}
	}
	...
	private function _updateSingleCellReference($pCellReference = 'A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0) {
		if (strpos($pCellReference, ':') === false && strpos($pCellReference, ',') === false) {
			...
			list($newColumn, $newRow) = _Cell::coordinateFromString( $pCellReference );

			// Verify which parts should be updated
			$updateColumn = (($newColumn{0} != '$') && ($beforeColumn{0} != '$') &&
							 _Cell::columnIndexFromString($newColumn) >= _Cell::columnIndexFromString($beforeColumn));
			...
			if ($updateColumn) {
				$newColumn	= _Cell::stringFromColumnIndex( _Cell::columnIndexFromString($newColumn) - 1 + $pNumCols );
			}
			...
			throw new _Exception("Only single cell references may be passed to this method.");
		}
	}
	...
	public final function __clone() {
		throw new _Exception("Cloning a Singleton is not allowed!");
	}
}
