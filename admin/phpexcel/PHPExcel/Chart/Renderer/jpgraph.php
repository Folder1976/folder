...
require_once(_Settings::getChartRendererPath().'/jpgraph.php');


/**
 * _Chart_Renderer_jpgraph
 *
 * @category	
 * @package		_Chart_Renderer
 * @copyright	Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Chart_Renderer_jpgraph
{
	...
	private function _formatDataSetLabels($groupID, $datasetLabels, $labelCount, $rotation = '') {
		...
		foreach($datasetLabels as $i => $datasetLabel) {
			if (is_array($datasetLabel)) {
				...
				if (!is_null($datasetLabelFormatCode)) {
					$datasetLabels[$i] = _Style_NumberFormat::toFormattedString($datasetLabel,$datasetLabelFormatCode);
				}
			}
			++$testCurrentIndex;
		}
	...
	/**
	 * Create a new _Chart_Renderer_jpgraph
	 */
	public function __construct(_Chart $chart)
	{
		$this->_graph	= null;
		$this->_chart	= $chart;
