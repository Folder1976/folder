 ...
 *
 * @category   
 * @package    _Writer_2007
 * @copyright  Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Writer_Excel2007 extends _Writer_Abstract implements _Writer_IWriter
{
	/**
	 ...
	 *
	 * @var _Writer_Excel2007_WriterPart[]
	 */
	...
	/**
	 * Private unique _Style_Conditional HashTable
	 *
	 * @var _HashTable
	 */
	...
	/**
	 * Private unique _Style_Fill HashTable
	 *
	 * @var _HashTable
	 */
	...
	/**
	 * Private unique _Style_Font HashTable
	 *
	 * @var _HashTable
	 */
	...
	/**
	 * Private unique _Style_Borders HashTable
	 *
	 * @var _HashTable
	 */
	...
	/**
	 * Private unique _Style_NumberFormat HashTable
	 *
	 * @var _HashTable
	 */
	...
	/**
	 * Private unique _Worksheet_BaseDrawing HashTable
	 *
	 * @var _HashTable
	 */
    ...
    /**
     * Create a new _Writer_Excel2007
     *
	 * @param 		$p
     */
    public function __construct( $p = null)
    {
    	...
    	$writerPartsArray = array(	'stringtable'	=> '_Writer_Excel2007_StringTable',
									'contenttypes'	=> '_Writer_Excel2007_ContentTypes',
									'docprops' 		=> '_Writer_Excel2007_DocProps',
									'rels'			=> '_Writer_Excel2007_Rels',
									'theme' 		=> '_Writer_Excel2007_Theme',
									'style' 		=> '_Writer_Excel2007_Style',
									'workbook' 		=> '_Writer_Excel2007_Workbook',
									'worksheet' 	=> '_Writer_Excel2007_Worksheet',
									'drawing' 		=> '_Writer_Excel2007_Drawing',
									'comments' 		=> '_Writer_Excel2007_Comments',
									'chart'			=> '_Writer_Excel2007_Chart',
								 );
	 ...
	 * @return 	_Writer_Excel2007_WriterPart
	 */
	...
	public function save($pFilename = null)
	{
		if ($this->_spreadSheet !== NULL) {
			...
			if (strtolower($pFilename) == 'php://output' || strtolower($pFilename) == 'php://stdout') {
				$pFilename = @tempnam(_Shared_File::sys_get_temp_dir(), 'phpxltmp');
				if ($pFilename == '') {
					$pFilename = $originalFilename;
				}
			}

			$saveDebugLog = _Calculation::getInstance($this->_spreadSheet)->getDebugLog()->getWriteDebugLog();
			_Calculation::getInstance($this->_spreadSheet)->getDebugLog()->setWriteDebugLog(FALSE);
			...
			$zipClass = _Settings::getZipClass();
			$objZip = new $zipClass();
			...
			if ($objZip->open($pFilename, $zipOverWrite) !== true) {
				if ($objZip->open($pFilename, $zipCreate) !== true) {
					throw new _Writer_Exception("Could not open " . $pFilename . " for writing.");
				}
			}
			...
			for ($i = 0; $i < $this->getDrawingHashTable()->count(); ++$i) {
				if ($this->getDrawingHashTable()->getByIndex($i) instanceof _Worksheet_Drawing) {
					$imageContents = null;
				...
				} else if ($this->getDrawingHashTable()->getByIndex($i) instanceof _Worksheet_MemoryDrawing) {
					ob_start();
					call_user_func(
			...
			_Calculation::getInstance($this->_spreadSheet)->getDebugLog()->setWriteDebugLog($saveDebugLog);

			// Close file
			if ($objZip->close() === false) {
				throw new _Writer_Exception("Could not close zip file $pFilename.");
			}

			// If a temporary file was used, copy it to the correct file stream
			if ($originalFilename != $pFilename) {
				if (copy($pFilename, $originalFilename) === false) {
					throw new _Writer_Exception("Could not copy temporary zip file $pFilename to $originalFilename.");
				}
				@unlink($pFilename);
			}
		} else {
			throw new _Writer_Exception(" object unassigned.");
		}
	}
	...
	public function get() {
		if ($this->_spreadSheet !== null) {
			...
			throw new _Writer_Exception("No  assigned.");
		}
	}
    ...
    /**
     * Get _Style_Conditional HashTable
     *
     * @return _HashTable
     */
    ...
    /**
     * Get _Style_NumberFormat HashTable
     *
     * @return _HashTable
     */
    ...
    /**
     * Get _Worksheet_BaseDrawing HashTable
     *
     * @return _HashTable
     */
