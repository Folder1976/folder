 ...
 * @copyright  Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Writer_Excel5 extends _Writer_Abstract implements _Writer_IWriter
{
	/**
	 *  object
	 *
	...
	public function __construct( $phpExcel) {
		...
		$this->_parser		= new _Writer_Excel5_Parser();
	}
	...
	public function save($pFilename = null) {
		...
		$saveDateReturnType = _Calculation_Functions::getReturnDateType();
		...
		$this->_writerWorkbook = new _Writer_Excel5_Workbook($this->_phpExcel,
		...
		for ($i = 0; $i < $countSheets; ++$i) {
			$this->_writerWorksheets[$i] = new _Writer_Excel5_Worksheet($this->_str_total, $this->_str_unique,
		...
		$OLE = new _Shared_OLE_PPS_File(_Shared_OLE::Asc2Ucs($workbookStreamName));
		...
		if(isset($this->_documentSummaryInformation) && !empty($this->_documentSummaryInformation)){
			$OLE_DocumentSummaryInformation = new _Shared_OLE_PPS_File(_Shared_OLE::Asc2Ucs(chr(5) . 'DocumentSummaryInformation'));
		...
		if(isset($this->_summaryInformation) && !empty($this->_summaryInformation)){
		  $OLE_SummaryInformation = new _Shared_OLE_PPS_File(_Shared_OLE::Asc2Ucs(chr(5) . 'SummaryInformation'));
		...
		$root = new _Shared_OLE_PPS_Root(time(), time(), $arrRootData);
		// save the OLE file
		$res = $root->save($pFilename);

		_Calculation_Functions::setReturnDateType($saveDateReturnType);
	...
	private function _buildWorksheetEschers()
	{
		...
		foreach ($this->_phpExcel->getAllsheets() as $sheet) {
			...
			$escher = new _Shared_Escher();

			// dgContainer
			$dgContainer = new _Shared_Escher_DgContainer();
			...
			// spgrContainer
			$spgrContainer = new _Shared_Escher_DgContainer_SpgrContainer();
			$dgContainer->setSpgrContainer($spgrContainer);

			// add one shape which is the group shape
			$spContainer = new _Shared_Escher_DgContainer_SpgrContainer_SpContainer();
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				...
				$twoAnchor = _Shared_Excel5::oneAnchor2twoAnchor($sheet, $coordinates, $offsetX, $offsetY, $width, $height);
			...
			if(!empty($filterRange)){
				$rangeBounds = _Cell::rangeBoundaries($filterRange);
				...
				while($iInc <= $iNumColEnd){
					...
					$oDrawing  = new _Worksheet_BaseDrawing();
					// get the coordinates of drawing
					$cDrawing   = _Cell::stringFromColumnIndex($iInc - 1) . $rangeBounds[0][1];
					...
					// set coordinates and offsets, client anchor
					$endCoordinates = _Cell::stringFromColumnIndex(_Cell::stringFromColumnIndex($iInc - 1));
	...
	private function _buildWorkbookEscher()
	{
		...
		// dggContainer
		$dggContainer = new _Shared_Escher_DggContainer();
		...
		// bstoreContainer
		$bstoreContainer = new _Shared_Escher_DggContainer_BstoreContainer();
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				if ($drawing instanceof _Worksheet_Drawing) {
					...
					case 1: // GIF, not supported by BIFF8, we convert to PNG
						$blipType = _Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
					...
					case 2: // JPEG
						$blipType = _Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
					...
					case 6: // Windows DIB (BMP), we convert to PNG
						$blipType = _Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
						imagepng(_Shared_Drawing::imagecreatefrombmp($filename));
						$blipData = ob_get_contents();
						ob_end_clean();
						break;
					...
					$blip = new _Shared_Escher_DggContainer_BstoreContainer_BSE_Blip();
					$blip->setData($blipData);

					$BSE = new _Shared_Escher_DggContainer_BstoreContainer_BSE();
					$BSE->setBlipType($blipType);
					$BSE->setBlip($blip);

					$bstoreContainer->addBSE($BSE);

				} else if ($drawing instanceof _Worksheet_MemoryDrawing) {

					switch ($drawing->getRenderingFunction()) {

					case _Worksheet_MemoryDrawing::RENDERING_JPEG:
						$blipType = _Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
						$renderingFunction = 'imagejpeg';
						break;

					case _Worksheet_MemoryDrawing::RENDERING_GIF:
					case _Worksheet_MemoryDrawing::RENDERING_PNG:
					case _Worksheet_MemoryDrawing::RENDERING_DEFAULT:
						$blipType = _Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
	...
	private function _writeSummaryInformation(){
		...
		if($this->_phpExcel->getProperties()->getCreated()){
			...
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0C),
								   ...
								   'data'	=> array('data' => _Shared_OLE::LocalDate2OLE($dataProp)));
			$dataSection_NumProps++;
		}
		//	Modified Date/Time
