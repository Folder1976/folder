...
/**
 * _Writer_Excel2007_ContentTypes
 *
 * @category   
 * @package    _Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Writer_Excel2007_ContentTypes extends _Writer_Excel2007_WriterPart
{
	/**
	 ...
	 */
	public function writeContentTypes( $p = null, $includeCharts = FALSE)
	{
		// Create XML writer
		$objWriter = null;
		...
		$objWriter->writeAttribute('xmlns', 'http://schemas.openxmlformats.org/package/2006/content-types');
			...
			$chart = 1;
			for ($i = 0; $i < $sheetCount; ++$i) {
				$drawings = $p->getSheet($i)->getDrawingCollection();
				$drawingCount = count($drawings);
				$chartCount = ($includeCharts) ? $p->getSheet($i)->getChartCount() : 0;
			...
			// Comments
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($p->getSheet($i)->getComments()) > 0) {
					$this->_writeOverrideContentType(
			...
			for ($i = 0; $i < $mediaCount; ++$i) {
				...
				if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof _Worksheet_Drawing) {
					$extension 	= strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
					$mimeType 	= $this->_getImageMimeType( $this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath() );
				} else if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof _Worksheet_MemoryDrawing) {
			...
			$sheetCount = $p->getSheetCount();
			for ($i = 0; $i < $sheetCount; ++$i) {
				if (count($p->getSheet()->getHeaderFooter()->getImages()) > 0) {
					foreach ($p->getSheet()->getHeaderFooter()->getImages() as $image) {
	...
	{
		if (_Shared_File::file_exists($pFile)) {
			$image = getimagesize($pFile);
			return image_type_to_mime_type($image[2]);
		} else {
			throw new _Writer_Exception("File $pFile does not exist");
		}
	}
	 ...
	 * @throws 	_Writer_Exception
	 */
	private function _writeDefaultContentType(_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
	 ...
	 * @throws 	_Writer_Exception
	 */
	private function _writeOverrideContentType(_Shared_XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
	{
		if ($pPartname != '' && $pContentType != '') {
			// Write content type
