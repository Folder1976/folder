...
/**
 * _Writer_Excel2007_Chart
 *
 * @category   
 * @package    _Writer_Excel2007
 * @copyright  Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Writer_Excel2007_Chart extends _Writer_Excel2007_WriterPart
{
	/**
	 ...
	 */
	public function writeChart(_Chart $pChart = null)
	{
		// Create XML writer
		$objWriter = null;
	 ...
	 * Write Chart Title
	 *
	 * @param	_Chart_Title		$title
	 * @param 	_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	_Writer_Exception
	 */
	private function _writeTitle(_Chart_Title $title = null, $objWriter)
	{
		if (is_null($title)) {
			return;
		}
	 ...
	 * Write Chart Legend
	 *
	 * @param	_Chart_Legend		$legend
	 * @param 	_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	_Writer_Exception
	 */
	private function _writeLegend(_Chart_Legend $legend = null, $objWriter)
	{
		if (is_null($legend)) {
			return;
		}
	 ...
	 *
	 * @param	_Chart_PlotArea		$plotArea
	 * @param	_Chart_Title		$xAxisLabel
	 * @param	_Chart_Title		$yAxisLabel
	 * @param 	_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	_Writer_Exception
	 */
	private function _writePlotArea(_Chart_PlotArea $plotArea,
									_Chart_Title $xAxisLabel = NULL,
									_Chart_Title $yAxisLabel = NULL,
									$objWriter,
									_Worksheet $pSheet)
	{
		if (is_null($plotArea)) {
				...
				$objWriter->startElement('c:'.$chartType);
					...
					for($i = 0; $i < $groupCount; ++$i) {
						...
						if ($groupType == $chartType) {
							...
							if ($groupType === _Chart_DataSeries::TYPE_RADARCHART) {
								$objWriter->startElement('c:radarStyle');
									$objWriter->writeAttribute('val', $plotStyle );
								$objWriter->endElement();
							} elseif ($groupType === _Chart_DataSeries::TYPE_SCATTERCHART) {
					...
					if ($chartType === _Chart_DataSeries::TYPE_LINECHART) {
						//	Line only, Line3D can't be smoothed

						$objWriter->startElement('c:smooth');
							$objWriter->writeAttribute('val', (integer) $plotGroup->getSmoothLine() );
						$objWriter->endElement();
					} elseif (($chartType === _Chart_DataSeries::TYPE_BARCHART) ||
						($chartType === _Chart_DataSeries::TYPE_BARCHART_3D)) {
						...
						if ($plotGroupingType == 'percentStacked' ||
					...
					} elseif ($chartType === _Chart_DataSeries::TYPE_BUBBLECHART) {
							...
							$objWriter->startElement('c:showNegBubbles');
								$objWriter->writeAttribute('val', 0 );
							$objWriter->endElement();
					} elseif ($chartType === _Chart_DataSeries::TYPE_STOCKCHART) {
...
//					} while ($id1 == $id2);
					...
					if (($chartType !== _Chart_DataSeries::TYPE_PIECHART) &&
						($chartType !== _Chart_DataSeries::TYPE_PIECHART_3D) &&
						($chartType !== _Chart_DataSeries::TYPE_DONUTCHART)) {
						...
						if ($chartType === _Chart_DataSeries::TYPE_DONUTCHART) {
			...
			if (($chartType !== _Chart_DataSeries::TYPE_PIECHART) &&
				...
				if ($chartType === _Chart_DataSeries::TYPE_BUBBLECHART) {
	 ...
	 *
	 * @param 	_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @param 	_Chart_Layout		$chartLayout	Chart layout
	 * @throws 	_Writer_Exception
	 */
	 ...
	 * @throws 	_Writer_Exception
	 */
	private function _writeCatAx($objWriter, _Chart_PlotArea $plotArea, $xAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
										$objWriter->writeRawData(_Shared_String::ControlCharacterPHP2OOXML( $caption ));
									$objWriter->endElement();
	 ...
	 * @throws 	_Writer_Exception
	 */
	private function _writeValAx($objWriter, _Chart_PlotArea $plotArea, $yAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries)
	{
...
//										$objWriter->writeAttribute('xml:space', 'preserve');
					...
					if ($groupType !== _Chart_DataSeries::TYPE_BUBBLECHART) {
	...
	private static function _getChartType($plotArea)
	{
		...
		if ($groupCount == 1) {
			...
			if (count($chartTypes) == 0) {
				throw new _Writer_Exception('Chart is not yet implemented');
			}
		}
	 ...
	 *
	 * @param	_Chart_DataSeries		$plotGroup
									...
									)
	{
		...
		if (($groupType == _Chart_DataSeries::TYPE_BARCHART) ||
			($groupType == _Chart_DataSeries::TYPE_BARCHART_3D)) {
		...
		if (($groupType !== _Chart_DataSeries::TYPE_RADARCHART) &&
			($groupType !== _Chart_DataSeries::TYPE_STOCKCHART)) {

			if ($groupType !== _Chart_DataSeries::TYPE_LINECHART) {
				if (($groupType == _Chart_DataSeries::TYPE_PIECHART) ||
					($groupType == _Chart_DataSeries::TYPE_PIECHART_3D) ||
					($groupType == _Chart_DataSeries::TYPE_DONUTCHART) ||
					($plotSeriesCount > 1)) {
		...
		foreach($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
			$objWriter->startElement('c:ser');
				...
				if (($groupType === _Chart_DataSeries::TYPE_BARCHART) ||
					($groupType === _Chart_DataSeries::TYPE_BARCHART_3D) ||
					($groupType === _Chart_DataSeries::TYPE_BUBBLECHART)) {
				...
				if ($plotSeriesCategory && ($plotSeriesCategory->getPointCount() > 0)) {
					...
					if (($groupType === _Chart_DataSeries::TYPE_BUBBLECHART) ||
						($groupType === _Chart_DataSeries::TYPE_SCATTERCHART)) {
	 ...
	 *
	 * @param	_Chart_DataSeriesValues		$plotSeriesLabel
	 * @param 	_Shared_XMLWriter 			$objWriter 			XML Writer
	 * @throws 	_Writer_Exception
	 */
	 ...
	 *
	 * @param	_Chart_DataSeriesValues		$plotSeriesValues
	 * @param 	_Shared_XMLWriter 			$objWriter 			XML Writer
	 * @param	string								$groupType			Type of plot for dataseries
	 ...
	 * @throws 	_Writer_Exception
	 */
	private function _writeBubbles($plotSeriesValues, $objWriter, _Worksheet $pSheet)
	{
		if (is_null($plotSeriesValues)) {
			return;
		}
	 ...
	 * Write Layout
	 *
	 * @param	_Chart_Layout		$layout
	 * @param 	_Shared_XMLWriter 	$objWriter 		XML Writer
	 * @throws 	_Writer_Exception
	 */
	private function _writeLayout(_Chart_Layout $layout = NULL, $objWriter)
	{
