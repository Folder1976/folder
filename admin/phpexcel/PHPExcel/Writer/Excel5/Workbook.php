...
/**
 * _Writer_Excel5_Workbook
 *
 * @category   
 * @package    _Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Writer_Excel5_Workbook extends _Writer_Excel5_BIFFwriter
{
	/**
	 * Formula parser
	 *
	 ...
	 */
	public function __construct( $phpExcel = null,
	...
	{
		$xfWriter = new _Writer_Excel5_Xf($style);
	 ...
	 *
	 * @param _Style_Font $font
	 * @return int Index to FONT record
	 */
	public function _addFont(_Style_Font $font)
	{
		...
		if(isset($this->_addedFonts[$fontHashCode])){
			...
			$fontWriter = new _Writer_Excel5_Font($font);
	...
	function _calcSheetOffsets()
	{
		...
		foreach ($this->_phpExcel->getWorksheetIterator() as $sheet) {
			$offset += $boundsheet_length + strlen(_Shared_String::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
		}
	...
	private function _writeNames()
	{
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isPrintAreaSet()) {
				// Print area
				$printArea = _Cell::splitRange($sheetSetup->getPrintArea());
				$printArea = $printArea[0];
				$printArea[0] = _Cell::coordinateFromString($printArea[0]);
				$printArea[1] = _Cell::coordinateFromString($printArea[1]);

				$print_rowmin = $printArea[0][1] - 1;
				$print_rowmax = $printArea[1][1] - 1;
				$print_colmin = _Cell::columnIndexFromString($printArea[0][0]) - 1;
				$print_colmax = _Cell::columnIndexFromString($printArea[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
				...
				$colmin = _Cell::columnIndexFromString($repeat[0]) - 1;
				$colmax = _Cell::columnIndexFromString($repeat[1]) - 1;
	...
	private function _writeAllDefinedNamesBiff8()
	{
		...
		if (count($this->_phpExcel->getNamedRanges()) > 0) {
			...
			foreach ($namedRanges as $namedRange) {
				...
				$range = _Cell::splitRange($namedRange->getRange());
				for ($i = 0; $i < count($range); $i++) {
					$range[$i][0] = '\'' . str_replace("'", "''", $namedRange->getWorksheet()->getTitle()) . '\'!' . _Cell::absoluteCoordinate($range[$i][0]);
					if (isset($range[$i][1])) {
						$range[$i][1] = _Cell::absoluteCoordinate($range[$i][1]);
					}
				}
				$range = _Cell::buildRange($range); // e.g. Sheet1!$A$1:$B$2
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if ($sheetSetup->isPrintAreaSet()) {
				...
				for ($j = 0; $j < $countPrintArea; ++$j) {
					...
					$printAreaRect[0] = _Cell::coordinateFromString($printAreaRect[0]);
					$printAreaRect[1] = _Cell::coordinateFromString($printAreaRect[1]);

					$print_rowmin = $printAreaRect[0][1] - 1;
					$print_rowmax = $printAreaRect[1][1] - 1;
					$print_colmin = _Cell::columnIndexFromString($printAreaRect[0][0]) - 1;
					$print_colmax = _Cell::columnIndexFromString($printAreaRect[1][0]) - 1;
		...
		for ($i = 0; $i < $total_worksheets; ++$i) {
			...
			if(!empty($autoFilterRange)) {
				$rangeBounds = _Cell::rangeBoundaries($autoFilterRange);
	...
	private function _writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
	{
		...
		$nlen = _Shared_String::CountCharacters($name);

		// name with stripped length field
		$name = substr(_Shared_String::UTF8toBIFF8UnicodeLong($name), 2);
	...
	private function _writeWindow1()
	{
		...
		// not supported by , so there is only one selected sheet, the active
		$ctabsel   = 1;       // Number of workbook tabs selected

		$wTabRatio = 0x0258;                 // Tab to scrollbar ratio

		// not supported by , set to 0
	 ...
	 *
	 * @param _Worksheet  $sheet Worksheet name
	...
	private function _writeBoundsheet($sheet, $offset)
	{
		...
		switch ($sheet->getSheetState()) {
			case _Worksheet::SHEETSTATE_VISIBLE:	$ss = 0x00; break;
			case _Worksheet::SHEETSTATE_HIDDEN:		$ss = 0x01; break;
			case _Worksheet::SHEETSTATE_VERYHIDDEN:	$ss = 0x02; break;
		...
		$data .= _Shared_String::UTF8toBIFF8UnicodeShort($sheetname);
	...
	private function _writeNumFormat($format, $ifmt)
	{
		...
		$numberFormatString = _Shared_String::UTF8toBIFF8UnicodeLong($format);
	...
	private function _writeDatemode()
	{
		...
		$f1904     = (_Shared_Date::getExcelCalendar() == _Shared_Date::CALENDAR_MAC_1904) ?
