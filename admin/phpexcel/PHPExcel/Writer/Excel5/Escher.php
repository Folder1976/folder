...
/**
 * _Shared_Escher_DggContainer_BstoreContainer
 *
 * @category   
 * @package    _Writer_Excel5
 * @copyright  Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Writer_Excel5_Escher
{
	/**
	...
	public function close()
	{
		...
		case '_Shared_Escher':
			if ($dggContainer = $this->_object->getDggContainer()) {
				$writer = new _Writer_Excel5_Escher($dggContainer);
				$this->_data = $writer->close();
			} else if ($dgContainer = $this->_object->getDgContainer()) {
				$writer = new _Writer_Excel5_Escher($dgContainer);
				$this->_data = $writer->close();
		...
		case '_Shared_Escher_DggContainer':
			...
			if ($bstoreContainer = $this->_object->getBstoreContainer()) {
				$writer = new _Writer_Excel5_Escher($bstoreContainer);
		...
		case '_Shared_Escher_DggContainer_BstoreContainer':
			// this is a container record
			...
			if ($BSECollection = $this->_object->getBSECollection()) {
				foreach ($BSECollection as $BSE) {
					$writer = new _Writer_Excel5_Escher($BSE);
					$innerData .= $writer->close();
				}
			}
		...
		case '_Shared_Escher_DggContainer_BstoreContainer_BSE':
			...
			if ($blip = $this->_object->getBlip()) {
				$writer = new _Writer_Excel5_Escher($blip);
				$innerData .= $writer->close();
			}
		...
		case '_Shared_Escher_DggContainer_BstoreContainer_BSE_Blip':
		...
		case '_Shared_Escher_DgContainer':
			// this is a container record
			...
			if ($spgrContainer = $this->_object->getSpgrContainer()) {
				$writer = new _Writer_Excel5_Escher($spgrContainer);
				$innerData .= $writer->close();
		...
		case '_Shared_Escher_DgContainer_SpgrContainer':
			// this is a container record
			...
			foreach ($this->_object->getChildren() as $spContainer) {
				$writer = new _Writer_Excel5_Escher($spContainer);
				$spData = $writer->close();
				$innerData .= $spData;
		...
		case '_Shared_Escher_DgContainer_SpgrContainer_SpContainer':
			// initialize
			$data = '';
			...
			if ($this->_object->getStartCoordinates()) {
				...
				// start coordinates
				list($column, $row) = _Cell::coordinateFromString($this->_object->getStartCoordinates());
				$c1 = _Cell::columnIndexFromString($column) - 1;
				$r1 = $row - 1;
				...
				// end coordinates
				list($column, $row) = _Cell::coordinateFromString($this->_object->getEndCoordinates());
				$c2 = _Cell::columnIndexFromString($column) - 1;
				$r2 = $row - 1;
