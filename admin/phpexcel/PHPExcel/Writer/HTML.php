 ...
 * @copyright  Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Writer_HTML extends _Writer_Abstract implements _Writer_IWriter {
	/**
	 *  object
	 *
	 * @var 
	...
	public function save($pFilename = null) {
		...
		$saveDebugLog = _Calculation::getInstance($this->_phpExcel)->getDebugLog()->getWriteDebugLog();
		_Calculation::getInstance($this->_phpExcel)->getDebugLog()->setWriteDebugLog(FALSE);
		$saveArrayReturnType = _Calculation::getArrayReturnType();
		_Calculation::setArrayReturnType(_Calculation::RETURN_ARRAY_AS_VALUE);
		...
		if ($fileHandle === false) {
			throw new _Writer_Exception("Could not open file $pFilename for writing.");
		}
		...
		_Calculation::setArrayReturnType($saveArrayReturnType);
		_Calculation::getInstance($this->_phpExcel)->getDebugLog()->setWriteDebugLog($saveDebugLog);
	}
	...
	private function _mapVAlign($vAlign) {
		switch ($vAlign) {
			case _Style_Alignment::VERTICAL_BOTTOM:		return 'bottom';
			case _Style_Alignment::VERTICAL_TOP:		return 'top';
			case _Style_Alignment::VERTICAL_CENTER:
			case _Style_Alignment::VERTICAL_JUSTIFY:	return 'middle';
	...
	private function _mapHAlign($hAlign) {
		switch ($hAlign) {
			case _Style_Alignment::HORIZONTAL_GENERAL:				return false;
			case _Style_Alignment::HORIZONTAL_LEFT:					return 'left';
			case _Style_Alignment::HORIZONTAL_RIGHT:				return 'right';
			case _Style_Alignment::HORIZONTAL_CENTER:
			case _Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS:	return 'center';
			case _Style_Alignment::HORIZONTAL_JUSTIFY:				return 'justify';
	...
	private function _mapBorderStyle($borderStyle) {
		switch ($borderStyle) {
			case _Style_Border::BORDER_NONE:				return 'none';
			case _Style_Border::BORDER_DASHDOT:				return '1px dashed';
			case _Style_Border::BORDER_DASHDOTDOT:			return '1px dotted';
			case _Style_Border::BORDER_DASHED:				return '1px dashed';
			case _Style_Border::BORDER_DOTTED:				return '1px dotted';
			case _Style_Border::BORDER_DOUBLE:				return '3px double';
			case _Style_Border::BORDER_HAIR:				return '1px solid';
			case _Style_Border::BORDER_MEDIUM:				return '2px solid';
			case _Style_Border::BORDER_MEDIUMDASHDOT:		return '2px dashed';
			case _Style_Border::BORDER_MEDIUMDASHDOTDOT:	return '2px dotted';
			case _Style_Border::BORDER_MEDIUMDASHED:		return '2px dashed';
			case _Style_Border::BORDER_SLANTDASHDOT:		return '2px dashed';
			case _Style_Border::BORDER_THICK:				return '3px solid';
			case _Style_Border::BORDER_THIN:				return '1px solid';
	...
	public function generateHTMLHeader($pIncludeStyles = false) {
		...
		if (is_null($this->_phpExcel)) {
			throw new _Writer_Exception('Internal  object not set to an instance of an object.');
		}
		...
		$html .= '<!-- Generated by  - http://www.phpexcel.net -->' . PHP_EOL;
		$html .= '<html>' . PHP_EOL;
		$html .= '  <head>' . PHP_EOL;
	...
	public function generateSheetData() {
		...
		foreach ($sheets as $sheet) {
			...
			$dimension[0] = _Cell::coordinateFromString($dimension[0]);
			$dimension[0][0] = _Cell::columnIndexFromString($dimension[0][0]) - 1;
			$dimension[1] = _Cell::coordinateFromString($dimension[1]);
			$dimension[1][0] = _Cell::columnIndexFromString($dimension[1][0]) - 1;
	...
	private function _extendRowsForChartsAndImages(_Worksheet $pSheet, $row) {
		$rowMax = $row;
		$colMax = 'A';
		if ($this->_includeCharts) {
			foreach ($pSheet->getChartCollection() as $chart) {
				if ($chart instanceof _Chart) {
				    $chartCoordinates = $chart->getTopLeftPosition();
				    $chartTL = _Cell::coordinateFromString($chartCoordinates['cell']);
					$chartCol = _Cell::columnIndexFromString($chartTL[0]);
					if ($chartTL[1] > $rowMax) {
						$rowMax = $chartTL[1];
						if ($chartCol > _Cell::columnIndexFromString($colMax)) {
							$colMax = $chartTL[0];
						}
					}
				}
		...
		foreach ($pSheet->getDrawingCollection() as $drawing) {
			if ($drawing instanceof _Worksheet_Drawing) {
			    $imageTL = _Cell::coordinateFromString($drawing->getCoordinates());
				$imageCol = _Cell::columnIndexFromString($imageTL[0]);
				if ($imageTL[1] > $rowMax) {
					$rowMax = $imageTL[1];
					if ($imageCol > _Cell::columnIndexFromString($colMax)) {
						$colMax = $imageTL[0];
					}
				}
			}
	 ...
	 *
	 * @param	_Worksheet	$pSheet			_Worksheet
	 * @param	string				$coordinates	Cell coordinates
	 * @return	string
	 * @throws	_Writer_Exception
	 */
	private function _writeImageInCell(_Worksheet $pSheet, $coordinates) {
		// Construct HTML
		$html = '';
	 ...
	 * @throws	_Writer_Exception
	 */
	private function _writeChartInCell(_Worksheet $pSheet, $coordinates) {
		// Construct HTML
		$html = '';
		...
		foreach ($pSheet->getChartCollection() as $chart) {
			if ($chart instanceof _Chart) {
				...
				if ($chartCoordinates['cell'] == $coordinates) {
					$chartFileName = _Shared_File::sys_get_temp_dir().'/'.uniqid().'.png';
	...
	public function buildCSS($generateSurroundingHTML = true) {
		...
		foreach ($sheets as $sheet) {
			...
			$highestColumnIndex = _Cell::columnIndexFromString($sheet->getHighestColumn()) - 1;
			$column = -1;
			...
			foreach ($sheet->getColumnDimensions() as $columnDimension) {
				if (($width = _Shared_Drawing::cellDimensionToPixels($columnDimension->getWidth(), $this->_defaultFont)) >= 0) {
					$width = _Shared_Drawing::pixelsToPoints($width);
					$column = _Cell::columnIndexFromString($columnDimension->getColumnIndex()) - 1;
			...
			if ($rowDimension->getRowHeight() == -1) {
				$pt_height = _Shared_Font::getDefaultRowHeightByFont($this->_phpExcel->getDefaultStyle()->getFont());
			} else {
	 ...
	 * Create CSS style
	 *
	 * @param	_Style		$pStyle			_Style
	 * @return	array
	 */
	private function _createCSSStyle(_Style $pStyle) {
		// Construct CSS
		$css = '';
	...
	/**
	 * Create CSS style (_Style_Alignment)
	 *
	 * @param	_Style_Alignment		$pStyle			_Style_Alignment
	 * @return	array
	 */
	private function _createCSSStyleAlignment(_Style_Alignment $pStyle) {
		// Construct CSS
		$css = array();
	...
	/**
	 * Create CSS style (_Style_Font)
	 *
	 * @param	_Style_Font		$pStyle			_Style_Font
	 * @return	array
	 */
	private function _createCSSStyleFont(_Style_Font $pStyle) {
		// Construct CSS
		$css = array();
		...
		if ($pStyle->getUnderline() != _Style_Font::UNDERLINE_NONE && $pStyle->getStrikethrough()) {
			$css['text-decoration'] = 'underline line-through';
		} else if ($pStyle->getUnderline() != _Style_Font::UNDERLINE_NONE) {
	...
	/**
	 * Create CSS style (_Style_Borders)
	 *
	 * @param	_Style_Borders		$pStyle			_Style_Borders
	 * @return	array
	 */
	private function _createCSSStyleBorders(_Style_Borders $pStyle) {
		// Construct CSS
		$css = array();
	...
	/**
	 * Create CSS style (_Style_Border)
	 *
	 * @param	_Style_Border		$pStyle			_Style_Border
	 * @return	string
	 */
	private function _createCSSStyleBorder(_Style_Border $pStyle) {
		// Create CSS
//		$css = $this->_mapBorderStyle($pStyle->getBorderStyle()) . ' #' . $pStyle->getColor()->getRGB();
	...
	/**
	 * Create CSS style (_Style_Fill)
	 *
	 * @param	_Style_Fill		$pStyle			_Style_Fill
	 * @return	array
	 */
	private function _createCSSStyleFill(_Style_Fill $pStyle) {
		// Construct HTML
		$css = array();

		// Create CSS
		$value = $pStyle->getFillType() == _Style_Fill::FILL_NONE ?
	 ...
	 *
	 * @param	_Worksheet	$pSheet		The worksheet for the table we are writing
	 * @return	string
	 * @throws	_Writer_Exception
	 */
	private function _generateTableHeader($pSheet) {
		...
		// Write <col> elements
		$highestColumnIndex = _Cell::columnIndexFromString($pSheet->getHighestColumn()) - 1;
		$i = -1;
	 ...
	 * @throws	_Writer_Exception
	 */
	private function _generateRow(_Worksheet $pSheet, $pValues = null, $pRow = 0) {
		if (is_array($pValues)) {
			// Construct HTML
			$html = '';
			...
			foreach ($pValues as $cell) {
				$coordinate = _Cell::stringFromColumnIndex($colNum) . ($pRow + 1);
				...
				if ($cell instanceof _Cell) {
					...
					if ($cell->getValue() instanceof _RichText) {
						...
						if ($this->_preCalculateFormulas) {
							$cellData = _Style_NumberFormat::toFormattedString(
					...
					if (!$this->_useInlineCss) {
						...
						if ($sharedStyle->getAlignment()->getHorizontal() == _Style_Alignment::HORIZONTAL_GENERAL
				...
				if (isset($this->_isBaseCell[$pSheet->getParent()->getIndex($pSheet)][$pRow + 1][$colNum])) {
					...
					$endCellCoord = _Cell::stringFromColumnIndex($colNum + $colSpan - 1) . ($pRow + $rowSpan);
				...
				if ($writeCell) {
					...
					$html .= '			<td';
						if (!$this->_useInlineCss) {
							...
							//** Necessary redundant code for the sake of _Writer_PDF **
	...
	private function _calculateSpans()
	{
		...
		foreach ($sheetIndexes as $sheetIndex) {
			...
			foreach ($sheet->getMergeCells() as $cells) {
				list($cells, ) = _Cell::splitRange($cells);
				$first = $cells[0];
				$last  = $cells[1];

				list($fc, $fr) = _Cell::coordinateFromString($first);
				$fc = _Cell::columnIndexFromString($fc) - 1;

				list($lc, $lr) = _Cell::coordinateFromString($last);
				$lc = _Cell::columnIndexFromString($lc) - 1;
			...
			$countColumns = _Cell::columnIndexFromString($sheet->getHighestColumn());
	...
	private function _setMargins(_Worksheet $pSheet) {
		$htmlPage = '@page { ';
		$htmlBody = 'body { ';

		$left = _Shared_String::FormatNumber($pSheet->getPageMargins()->getLeft()) . 'in; ';
		$htmlPage .= 'left-margin: ' . $left;
		$htmlBody .= 'left-margin: ' . $left;
		$right = _Shared_String::FormatNumber($pSheet->getPageMargins()->getRight()) . 'in; ';
		$htmlPage .= 'right-margin: ' . $right;
		$htmlBody .= 'right-margin: ' . $right;
		$top = _Shared_String::FormatNumber($pSheet->getPageMargins()->getTop()) . 'in; ';
		$htmlPage .= 'top-margin: ' . $top;
		$htmlBody .= 'top-margin: ' . $top;
		$bottom = _Shared_String::FormatNumber($pSheet->getPageMargins()->getBottom()) . 'in; ';
