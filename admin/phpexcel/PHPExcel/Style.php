 ...
 * @copyright  Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Style extends _Style_Supervisor implements _IComparable
{
    /**
     * Font
     *
    ...
    public function __construct($isSupervisor = false, $isConditional = false)
    {
        ...
        $this->_font              = new _Style_Font($isSupervisor, $isConditional);
        $this->_fill              = new _Style_Fill($isSupervisor, $isConditional);
        $this->_borders           = new _Style_Borders($isSupervisor, $isConditional);
        $this->_alignment         = new _Style_Alignment($isSupervisor, $isConditional);
        $this->_numberFormat      = new _Style_NumberFormat($isSupervisor, $isConditional);
        $this->_protection        = new _Style_Protection($isSupervisor, $isConditional);
     ...
     *                 'bold'      => true,
     *                 'italic'    => false,
     *                 'underline' => _Style_Font::UNDERLINE_DOUBLE,
     *                 'strike'    => false,
     *                 'color'     => array(
     *                     'rgb' => '808080'
     *                 )
            ...
            if ($this->_isSupervisor) {
                ...
                if ($pAdvanced && isset($pStyles['borders'])) {
                    ...
                    for ($x = 1; $x <= $xMax; ++$x) {
                        ...
                        $colStart = ($x == 3) ?
                            _Cell::stringFromColumnIndex($rangeEnd[0])
                                : _Cell::stringFromColumnIndex($rangeStart[0] + $x - 1);

                        // end column index for region
                        $colEnd = ($x == 1) ?
                            _Cell::stringFromColumnIndex($rangeStart[0])
                                : _Cell::stringFromColumnIndex($rangeEnd[0] - $xMax + $x);
     ...
     * @return _Style
     */
    public function setFont(_Style_Font $font)
    {
        $this->_font = $font;
        return $this;
    }
     ...
     *
     * @param _Style_Conditional[] $pValue Array of condtional styles
     * @return _Style
     */
