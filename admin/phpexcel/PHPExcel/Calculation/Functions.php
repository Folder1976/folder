...
class _Calculation_Functions {
	 ...
	 *													_Calculation_Functions::COMPATIBILITY_EXCEL			'Excel'
	 *													_Calculation_Functions::COMPATIBILITY_GNUMERIC		'Gnumeric'
	 *													_Calculation_Functions::COMPATIBILITY_OPENOFFICE	'OpenOfficeCalc'
	 ...
	 *												Permitted values are:
	 *													_Calculation_Functions::RETURNDATE_PHP_NUMERIC		'P'
	 *													_Calculation_Functions::RETURNDATE_PHP_OBJECT		'O'
	 *													_Calculation_Functions::RETURNDATE_EXCEL			'E'
	 * @return	 boolean							Success or failure
	 */
	...
	public static function _ifCondition($condition) {
		$condition	= _Calculation_Functions::flattenSingleValue($condition);
		if (!isset($condition{0}))
			$condition = '=""';
		if (!in_array($condition{0},array('>', '<', '='))) {
			if (!is_numeric($condition)) { $condition = _Calculation::_wrapResult(strtoupper($condition)); }
			return '='.$condition;
		} else {
			preg_match('/([<>=]+)(.*)/',$condition,$matches);
			list(,$operator,$operand) = $matches;
			if (!is_numeric($operand)) { $operand = _Calculation::_wrapResult(strtoupper($operand)); }
