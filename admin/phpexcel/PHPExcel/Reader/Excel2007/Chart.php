...
/**
 * _Reader_Excel2007_Chart
 *
 * @category	
 * @package		_Reader_Excel2007
 * @copyright	Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Reader_Excel2007_Chart
{
						...
						switch ($chartDetailsKey) {
							case "plotArea":
									...
									if ($plotAreaLayout == NULL) {
										$plotAreaLayout = new _Chart_Layout();
									}
									$plotArea = new _Chart_PlotArea($plotAreaLayout,$plotSeries);
							...
							case "legend":
									...
									$legend = new _Chart_Legend($legendPos, $legendLayout, $legendOverlay);
									break;
						}
					}
			}
		}
		$chart = new _Chart($chartName,$title,$legend,$plotArea,$plotVisOnly,$dispBlanksAs,$XaxisLabel,$YaxisLabel);
	...
	private static function _chartTitle($titleDetails,$namespacesChartMeta,$type) {
		...
		return new _Chart_Title($caption, $titleLayout);
...
//			echo $detailKey,' => ',self::_getAttribute($detail, 'val', 'string'),PHP_EOL;
		...
		return new _Chart_Layout($layout);
	...
	private static function _chartDataSeries($chartDetail,$namespacesChartMeta,$plotType) {
		...
		return new _Chart_DataSeries($plotType,$multiSeriesType,$plotOrder,$seriesLabel,$seriesCategory,$seriesValues,$smoothLine);
	...
	private static function _chartDataSeriesValueSet($seriesDetail, $namespacesChartMeta, $marker = null, $smoothLine = false) {
		if (isset($seriesDetail->strRef)) {
			...
			return new _Chart_DataSeriesValues('String',$seriesSource,$seriesData['formatCode'],$seriesData['pointCount'],$seriesData['dataValues'],$marker,$smoothLine);
		} elseif (isset($seriesDetail->numRef)) {
			$seriesSource = (string) $seriesDetail->numRef->f;
			$seriesData = self::_chartDataSeriesValues($seriesDetail->numRef->numCache->children($namespacesChartMeta['c']));

			return new _Chart_DataSeriesValues('Number',$seriesSource,$seriesData['formatCode'],$seriesData['pointCount'],$seriesData['dataValues'],$marker,$smoothLine);
		...
		foreach($titleDetailPart as $titleDetailElementKey => $titleDetailElement) {
			...
			if (isset($titleDetailElement->rPr)) {
				...
				if (!is_null($fontColor)) {
					$objText->getFont()->setColor( new _Style_Color( self::_readColor($fontColor) ) );
				}
				...
				if (!is_null($underscore)) {
					if ($underscore == 'sng') {
						...
						$objText->getFont()->setUnderline(_Style_Font::UNDERLINE_DOUBLE);
					} else {
						$objText->getFont()->setUnderline(_Style_Font::UNDERLINE_NONE);
					}
				}
