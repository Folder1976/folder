 ...
 * @copyright  Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Reader_CSV extends _Reader_Abstract implements _Reader_IReader
{
	/**
	 * Input encoding
	 *
	...
	public function loadIntoExisting($pFilename,  $obj)
	{
		...
		$sheet = $obj->setActiveSheetIndex($this->_sheetIndex);
			...
			foreach($rowData as $rowDatum) {
				if ($rowDatum != '' && $this->_readFilter->readCell($columnLetter, $currentRow)) {
					...
					if ($this->_inputEncoding !== 'UTF-8') {
						$rowDatum = _Shared_String::ConvertEncoding($rowDatum, 'UTF-8', $this->_inputEncoding);
					}
