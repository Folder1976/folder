 ...
 * @copyright	Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Reader_Excel5 extends _Reader_Abstract implements _Reader_IReader
{
	// ParseXL definitions
	...
	/**
	 * Create a new _Reader_Excel5 instance
	 */
	public function __construct() {
		$this->_readFilter = new _Reader_DefaultReadFilter();
	}
	...
	public function canRead($pFilename)
	{
		...
		if (!file_exists($pFilename)) {
			throw new _Reader_Exception("Could not open " . $pFilename . " for reading! File does not exist.");
		}

		try {
			// Use ParseXL for the hard work.
			$ole = new _Shared_OLERead();
	...
	/**
	 * Reads names of the worksheets from a file, without parsing the whole file to a  object
	 *
	 * @param 	string 		$pFilename
	 * @throws 	_Reader_Exception
	 */
	...
	public function listWorksheetInfo($pFilename)
	{
		...
		foreach ($this->_sheets as $sheet) {
			...
			$tmpInfo['lastColumnLetter'] = _Cell::stringFromColumnIndex($tmpInfo['lastColumnIndex']);
	...
	public function load($pFilename)
	{
		...
		if (!$this->_readDataOnly && $this->_drawingGroupData) {
			$escherWorkbook = new _Shared_Escher();
			$reader = new _Reader_Excel5_Escher($escherWorkbook);
			$escherWorkbook = $reader->load($this->_drawingGroupData);

			// debug Escher stream
			//$debug = new Debug_Escher(new _Shared_Escher());
		...
		foreach ($this->_sheets as $sheet) {
			...
			if (!$this->_readDataOnly && $this->_drawingData) {
				$escherWorksheet = new _Shared_Escher();
				$reader = new _Reader_Excel5_Escher($escherWorksheet);
...
//				echo '<br />';
				...
				if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
					...
					list($startColumn, $startRow) = _Cell::coordinateFromString($spContainer->getStartCoordinates());
					list($endColumn, $endRow) = _Cell::coordinateFromString($spContainer->getEndCoordinates());
					...
					$width = _Shared_Excel5::getDistanceX($this->_phpSheet, $startColumn, $startOffsetX, $endColumn, $endOffsetX);
					$height = _Shared_Excel5::getDistanceY($this->_phpSheet, $startRow, $startOffsetY, $endRow, $endOffsetY);

					// calculate offsetX and offsetY of the shape
					$offsetX = $startOffsetX * _Shared_Excel5::sizeCol($this->_phpSheet, $startColumn) / 1024;
					$offsetY = $startOffsetY * _Shared_Excel5::sizeRow($this->_phpSheet, $startRow) / 256;
...
//							echo 'Picture Object<br />';
							...
							if ($blip = $BSE->getBlip()) {
								...
								$drawing = new _Worksheet_MemoryDrawing();
								...
								switch ($blipType) {
									case _Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG:
										$drawing->setRenderingFunction(_Worksheet_MemoryDrawing::RENDERING_JPEG);
										$drawing->setMimeType(_Worksheet_MemoryDrawing::MIMETYPE_JPEG);
										break;

									case _Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG:
										$drawing->setRenderingFunction(_Worksheet_MemoryDrawing::RENDERING_PNG);
										$drawing->setMimeType(_Worksheet_MemoryDrawing::MIMETYPE_PNG);
										break;
								}
			...
			if ($this->_version == self::XLS_BIFF8) {
				foreach ($this->_sharedFormulaParts as $cell => $baseCell) {
					list($column, $row) = _Cell::coordinateFromString($cell);
					if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($column, $row, $this->_phpSheet->getTitle()) ) {
						...
						$this->_phpSheet->getCell($cell)->setValueExplicit('=' . $formula, _Cell_DataType::TYPE_FORMULA);
					}
				}
			}
			...
			if ($definedName['isBuiltInName']) {
						...
						if (count($explodes) == 2) {
							if ($docSheet = $this->_phpExcel->getSheetByName($explodes[0])) {
								...
								if (count($coordinateStrings) == 2) {
									list($firstColumn, $firstRow) = _Cell::coordinateFromString($coordinateStrings[0]);
									list($lastColumn, $lastRow) = _Cell::coordinateFromString($coordinateStrings[1]);
				...
				if (count($explodes) == 2) {
					if (($docSheet = $this->_phpExcel->getSheetByName($explodes[0])) ||
						...
						$this->_phpExcel->addNamedRange( new _NamedRange((string)$definedName['name'], $docSheet, $extractedRange, $localOnly, $scope) );
					}
				} else {
					//	Named Value
		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($type) {
				...
				case 0x1E: // null-terminated string prepended by dword string length
					...
					$value = _Shared_String::ConvertEncoding($value, 'UTF-8', $codePage);
					$value = rtrim($value);
					break;

				case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-time
					$value = _Shared_OLE::OLE2LocalDate(substr($this->_summaryInformation, $secOffset + 4 + $offset, 8));
					break;
			...
			switch ($id) {
				case 0x01:	//	Code Page
					$codePage = _Shared_CodePage::NumberToName($value);
					break;
...
//			echo 'Type is ',$type,', ';
			...
			switch ($type) {
				...
				case 0x40:	//	Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
					// PHP-Time
					$value = _Shared_OLE::OLE2LocalDate(substr($this->_documentSummaryInformation, $secOffset + 4 + $offset, 8));
					break;
		...
		switch ($substreamType) {
			case self::XLS_WorkbookGlobals:
				...
				if (($version != self::XLS_BIFF8) && ($version != self::XLS_BIFF7)) {
					throw new _Reader_Exception('Cannot read this Excel file. Version is too old.');
				}
...
//		$recordData = substr($this->_data, $this->_pos + 4, $length);
		...
		throw new _Reader_Exception('Cannot read encrypted file');
	}
	...
	private function _readCodepage()
	{
		...
		$this->_codepage = _Shared_CodePage::NumberToName($codepage);
	}
	...
	private function _readDateMode()
	{
		...
		_Shared_Date::setExcelCalendar(_Shared_Date::CALENDAR_WINDOWS_1900);
		if (ord($recordData{0}) == 1) {
			_Shared_Date::setExcelCalendar(_Shared_Date::CALENDAR_MAC_1904);
		}
	}
		...
		if (!$this->_readDataOnly) {
			...
			switch ($underlineType) {
				case 0x00:
					break; // no underline
				case 0x01:
					$objFont->setUnderline(_Style_Font::UNDERLINE_SINGLE);
					break;
				case 0x02:
					$objFont->setUnderline(_Style_Font::UNDERLINE_DOUBLE);
					break;
				case 0x21:
					$objFont->setUnderline(_Style_Font::UNDERLINE_SINGLEACCOUNTING);
					break;
				case 0x22:
					$objFont->setUnderline(_Style_Font::UNDERLINE_DOUBLEACCOUNTING);
					break;
			}
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			} elseif (($code = _Style_NumberFormat::builtInFormatCode($numberFormatIndex)) !== '') {
			...
			$objStyle->getProtection()->setLocked($isLocked ?
				_Style_Protection::PROTECTION_INHERIT : _Style_Protection::PROTECTION_UNPROTECTED);

			// bit 1; mask 0x02; 1 = Formula is hidden
			$isHidden = (0x02 & $xfTypeProt) >> 1;
			$objStyle->getProtection()->setHidden($isHidden ?
				_Style_Protection::PROTECTION_PROTECTED : _Style_Protection::PROTECTION_UNPROTECTED);
			...
			switch ($horAlign) {
				case 0:
					$objStyle->getAlignment()->setHorizontal(_Style_Alignment::HORIZONTAL_GENERAL);
					break;
				case 1:
					$objStyle->getAlignment()->setHorizontal(_Style_Alignment::HORIZONTAL_LEFT);
					break;
				case 2:
					$objStyle->getAlignment()->setHorizontal(_Style_Alignment::HORIZONTAL_CENTER);
					break;
				case 3:
					$objStyle->getAlignment()->setHorizontal(_Style_Alignment::HORIZONTAL_RIGHT);
					break;
				case 5:
					$objStyle->getAlignment()->setHorizontal(_Style_Alignment::HORIZONTAL_JUSTIFY);
					break;
				case 6:
					$objStyle->getAlignment()->setHorizontal(_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS);
					break;
			}
			...
			switch ($vertAlign) {
				case 0:
					$objStyle->getAlignment()->setVertical(_Style_Alignment::VERTICAL_TOP);
					break;
				case 1:
					$objStyle->getAlignment()->setVertical(_Style_Alignment::VERTICAL_CENTER);
					break;
				case 2:
					$objStyle->getAlignment()->setVertical(_Style_Alignment::VERTICAL_BOTTOM);
					break;
				case 3:
					$objStyle->getAlignment()->setVertical(_Style_Alignment::VERTICAL_JUSTIFY);
					break;
			}
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				// offset: 10; size: 4; Cell border lines and background area
					...
					if ($diagonalUp == false && $diagonalDown == false) {
						$objStyle->getBorders()->setDiagonalDirection(_Style_Borders::DIAGONAL_NONE);
					} elseif ($diagonalUp == true && $diagonalDown == false) {
						$objStyle->getBorders()->setDiagonalDirection(_Style_Borders::DIAGONAL_UP);
					} elseif ($diagonalUp == false && $diagonalDown == true) {
						$objStyle->getBorders()->setDiagonalDirection(_Style_Borders::DIAGONAL_DOWN);
					} elseif ($diagonalUp == true && $diagonalDown == true) {
						$objStyle->getBorders()->setDiagonalDirection(_Style_Borders::DIAGONAL_BOTH);
					}
	...
	private function _readSheet()
	{
		...
		switch (ord($recordData{4})) {
			case 0x00: $sheetState = _Worksheet::SHEETSTATE_VISIBLE;    break;
			case 0x01: $sheetState = _Worksheet::SHEETSTATE_HIDDEN;     break;
			case 0x02: $sheetState = _Worksheet::SHEETSTATE_VERYHIDDEN; break;
			default: $sheetState = _Worksheet::SHEETSTATE_VISIBLE;      break;
		}
	...
	private function _readHorizontalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($cf, $r, _Worksheet::BREAK_ROW);
			}
		}
	}
	...
	private function _readVerticalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				...
				$this->_phpSheet->setBreakByColumnAndRow($c, $rf, _Worksheet::BREAK_COLUMN);
			}
		}
	}
	...
	private function _readPageSetup()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if (!$isNotInit) {
				...
				switch ($isPortrait) {
				case 0: $this->_phpSheet->getPageSetup()->setOrientation(_Worksheet_PageSetup::ORIENTATION_LANDSCAPE); break;
				case 1: $this->_phpSheet->getPageSetup()->setOrientation(_Worksheet_PageSetup::ORIENTATION_PORTRAIT); break;
				}
	...
	private function _readRk()
	{
		...
		$columnString = _Cell::stringFromColumnIndex($column);
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			// add cell
			$cell->setValueExplicit($numValue, _Cell_DataType::TYPE_NUMERIC);
		}
	}
	...
	private function _readLabelSst()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				$richText = new _RichText();
				$charPos = 0;
				$sstCount = count($this->_sst[$index]['fmtRuns']);
				for ($i = 0; $i <= $sstCount; ++$i) {
					if (isset($fmtRuns[$i])) {
						$text = _Shared_String::Substring($this->_sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
						$charPos = $fmtRuns[$i]['charPos'];
					} else {
						$text = _Shared_String::Substring($this->_sst[$index]['value'], $charPos, _Shared_String::CountCharacters($this->_sst[$index]['value']));
					}

					if (_Shared_String::CountCharacters($text) > 0) {
				...
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				$cell->setValueExplicit($richText, _Cell_DataType::TYPE_STRING);
			} else {
				$cell = $this->_phpSheet->getCell($columnString . ($row + 1));
				$cell->setValueExplicit($this->_sst[$index]['value'], _Cell_DataType::TYPE_STRING);
			}
	...
	private function _readMulRk()
	{
		...
		for ($i = 0; $i < $columns; ++$i) {
			$columnString = _Cell::stringFromColumnIndex($colFirst + $i);
	...
	private function _readFormula()
	{
		...
		if ($isPartOfSharedFormula) {
			...
			$this->_baseCell = _Cell::stringFromColumnIndex($baseCol). ($baseRow + 1);
		}
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if ( (ord($recordData{6}) == 0)
				...
				$dataType = _Cell_DataType::TYPE_STRING;
				...
				$dataType = _Cell_DataType::TYPE_BOOL;
				...
				$dataType = _Cell_DataType::TYPE_ERROR;
				...
				$dataType = _Cell_DataType::TYPE_NULL;
				$value = '';

			} else {

				// forumla result is a number, first 14 bytes like _NUMBER record
				$dataType = _Cell_DataType::TYPE_NUMERIC;
			...
			if (!$isPartOfSharedFormula) {
				...
				try {
					if ($this->_version != self::XLS_BIFF8) {
						throw new _Reader_Exception('Not BIFF8. Can only read BIFF8 formulas');
					}
					$formula = $this->_getFormulaFromStructure($formulaStructure); // get formula in human language
					$cell->setValueExplicit('=' . $formula, _Cell_DataType::TYPE_FORMULA);
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			switch ($isError) {
				case 0: // boolean
					...
					// add cell value
					$cell->setValueExplicit($value, _Cell_DataType::TYPE_BOOL);
					break;
				...
				case 1: // error type
					...
					// add cell value
					$cell->setValueExplicit($value, _Cell_DataType::TYPE_ERROR);
					break;
			}
	...
	private function _readMulBlank()
	{
		...
		if (!$this->_readDataOnly) {
			for ($i = 0; $i < $length / 2 - 3; ++$i) {
				$columnString = _Cell::stringFromColumnIndex($fc + $i);
	...
	private function _readBlank()
	{
		...
		$columnString = _Cell::stringFromColumnIndex($col);
...
//		echo '<br />';
	...
	private function _readWindow2()
	{
		...
		if ($this->_phpSheet->getSheetView()->getView() !== _Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT) {
			...
			$view = $isPageBreakPreview? _Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW :
				_Worksheet_SheetView::SHEETVIEW_NORMAL;
	...
	private function _readPageLayoutView(){
		...
		if ($fPageLayoutView === 1) {
			$this->_phpSheet->getSheetView()->setView(_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT);
	...
	private function _readPane()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_frozen) {
				// frozen panes
				$this->_phpSheet->freezePane(_Cell::stringFromColumnIndex($px) . ($py + 1));
			} else {
				// unfrozen panes; split windows; not supported by  core
			}
		}
	}
	...
	private function _includeCellRangeFiltered($cellRangeAddress)
	{
		...
		if ($this->getReadFilter() !== NULL) {
			...
			$rangeBoundaries = _Cell::getRangeBoundaries($cellRangeAddress);
			$rangeBoundaries[1][0]++;
	...
	private function _readHyperLink()
	{
		...
		if (!$this->_readDataOnly) {
			...
			foreach (_Cell::extractAllCellReferencesInRange($cellRange) as $coordinate) {
	...
	private function _readDataValidation()
	{
		...
		switch ($type) {
			case 0x00:	$type = _Cell_DataValidation::TYPE_NONE;		break;
			case 0x01:	$type = _Cell_DataValidation::TYPE_WHOLE;		break;
			case 0x02:	$type = _Cell_DataValidation::TYPE_DECIMAL;		break;
			case 0x03:	$type = _Cell_DataValidation::TYPE_LIST;		break;
			case 0x04:	$type = _Cell_DataValidation::TYPE_DATE;		break;
			case 0x05:	$type = _Cell_DataValidation::TYPE_TIME;		break;
			case 0x06:	$type = _Cell_DataValidation::TYPE_TEXTLENGTH;	break;
			case 0x07:	$type = _Cell_DataValidation::TYPE_CUSTOM;		break;
		}

		// bit: 4-6; mask: 0x00000070; error type
		$errorStyle = (0x00000070 & $options) >> 4;
		switch ($errorStyle) {
			case 0x00:	$errorStyle = _Cell_DataValidation::STYLE_STOP;			break;
			case 0x01:	$errorStyle = _Cell_DataValidation::STYLE_WARNING;		break;
			case 0x02:	$errorStyle = _Cell_DataValidation::STYLE_INFORMATION;	break;
		}
		...
		switch ($operator) {
			case 0x00: $operator = _Cell_DataValidation::OPERATOR_BETWEEN			;	break;
			case 0x01: $operator = _Cell_DataValidation::OPERATOR_NOTBETWEEN		;	break;
			case 0x02: $operator = _Cell_DataValidation::OPERATOR_EQUAL				;	break;
			case 0x03: $operator = _Cell_DataValidation::OPERATOR_NOTEQUAL			;	break;
			case 0x04: $operator = _Cell_DataValidation::OPERATOR_GREATERTHAN		;	break;
			case 0x05: $operator = _Cell_DataValidation::OPERATOR_LESSTHAN			;	break;
			case 0x06: $operator = _Cell_DataValidation::OPERATOR_GREATERTHANOREQUAL;	break;
			case 0x07: $operator = _Cell_DataValidation::OPERATOR_LESSTHANOREQUAL	;	break;
		}
		...
		try {
			...
			if ($type == _Cell_DataValidation::TYPE_LIST) {
		...
		foreach ($cellRangeAddresses as $cellRange) {
			...
			$stRange = _Cell::extractAllCellReferencesInRange($stRange);
...
//			var_dump($bcBitCount);
			...
			$drawing = new _Worksheet_Drawing();
	...
	private function _getNextToken($formulaData, $baseCell = 'A1')
	{
		...
		case 0x19:	//	Special attribute
			...
			case 0x41:
				...
				case 0x05:
					...
					break;
				default:
					throw new _Reader_Exception('Unrecognized space type in tAttrSpace token');
					break;
				}
				...
				break;
			default:
				throw new _Reader_Exception('Unrecognized attribute flag in tAttr token');
				break;
			}
			break;
		...
		case 0x61:
			...
			default:
				throw new _Reader_Exception('Unrecognized function in formula');
				break;
			}
		...
		default:
			throw new _Reader_Exception('Unrecognized token ' . sprintf('%02X', $id) . ' in formula');
			break;
		}
	...
	private function _readBIFF8CellAddress($cellAddressStructure)
	{
		...
		// offset: 2; size: 2; index to column or column offset + relative flags
			...
			$column = _Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($cellAddressStructure, 2));
	...
	{
		list($baseCol, $baseRow) = _Cell::coordinateFromString($baseCell);
		$baseCol = _Cell::columnIndexFromString($baseCol) - 1;
		...
		// offset: 2; size: 2; index to column or column offset + relative flags
			...
			// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
			if (!(0x4000 & self::_GetInt2d($cellAddressStructure, 2))) {
				$column = _Cell::stringFromColumnIndex($colIndex);
				$column = '$' . $column;
			} else {
				$colIndex = ($colIndex <= 127) ? $colIndex : $colIndex - 256;
				$column = _Cell::stringFromColumnIndex($baseCol + $colIndex);
			}
	...
	private function _readBIFF5CellRangeAddressFixed($subData)
	{
		...
		// check values
		if ($fr > $lr || $fc > $lc) {
			throw new _Reader_Exception('Not a cell range address');
		}

		// column index to letter
		$fc = _Cell::stringFromColumnIndex($fc);
		$lc = _Cell::stringFromColumnIndex($lc);
	...
	private function _readBIFF8CellRangeAddress($subData)
	{
		...
		// bit: 7-0; mask 0x00FF; column index
		$fc = _Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($subData, 4));
		...
		// bit: 7-0; mask 0x00FF; column index
		$lc = _Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($subData, 6));
	...
	private function _readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
	{
		...
		if (!(0x4000 & self::_GetInt2d($subData, 4))) {
			...
			$fc = _Cell::stringFromColumnIndex($fcIndex);
			$fc = '$' . $fc;
		} else {
			// column offset
			$fcIndex = ($fcIndex <= 127) ? $fcIndex : $fcIndex - 256;
			$fc = _Cell::stringFromColumnIndex($baseCol + $fcIndex);
		}
		...
		$lc = _Cell::stringFromColumnIndex($baseCol + $lcIndex);

		// bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
		if (!(0x4000 & self::_GetInt2d($subData, 6))) {
			// absolute column index
			$lc = _Cell::stringFromColumnIndex($lcIndex);
			$lc = '$' . $lc;
		} else {
			// column offset
			$lcIndex = ($lcIndex <= 127) ? $lcIndex : $lcIndex - 256;
	 ...
	 * in which case an _Reader_Exception is thrown
	 *
	 * @param int $index
	 * @return string|false
	 * @throws _Reader_Exception
			...
			switch ($type) {
				case 'internal':
					...
					if ($this->_ref[$index]['firstSheetIndex'] == 0xFFFF or $this->_ref[$index]['lastSheetIndex'] == 0xFFFF) {
						throw new _Reader_Exception('Deleted sheet reference');
					}
				...
				default:
					...
					throw new _Reader_Exception('Excel5 reader only supports internal sheets in fomulas');
					break;
			}
		}
		return false;
	...
	private static function _encodeUTF16($string, $compressed = '')
	{
		...
		return _Shared_String::ConvertEncoding($string, 'UTF-8', 'UTF-16LE');
	}
	...
	{
		return _Shared_String::ConvertEncoding($string, 'UTF-8', $this->_codepage);
	}
	...
	{
		switch ($index) {
			case 0x00: return _Style_Border::BORDER_NONE;
			case 0x01: return _Style_Border::BORDER_THIN;
			case 0x02: return _Style_Border::BORDER_MEDIUM;
			case 0x03: return _Style_Border::BORDER_DASHED;
			case 0x04: return _Style_Border::BORDER_DOTTED;
			case 0x05: return _Style_Border::BORDER_THICK;
			case 0x06: return _Style_Border::BORDER_DOUBLE;
			case 0x07: return _Style_Border::BORDER_HAIR;
			case 0x08: return _Style_Border::BORDER_MEDIUMDASHED;
			case 0x09: return _Style_Border::BORDER_DASHDOT;
			case 0x0A: return _Style_Border::BORDER_MEDIUMDASHDOT;
			case 0x0B: return _Style_Border::BORDER_DASHDOTDOT;
			case 0x0C: return _Style_Border::BORDER_MEDIUMDASHDOTDOT;
			case 0x0D: return _Style_Border::BORDER_SLANTDASHDOT;
			default:   return _Style_Border::BORDER_NONE;
		}
	}
	...
	{
		switch ($index) {
			case 0x00: return _Style_Fill::FILL_NONE;
			case 0x01: return _Style_Fill::FILL_SOLID;
			case 0x02: return _Style_Fill::FILL_PATTERN_MEDIUMGRAY;
			case 0x03: return _Style_Fill::FILL_PATTERN_DARKGRAY;
			case 0x04: return _Style_Fill::FILL_PATTERN_LIGHTGRAY;
			case 0x05: return _Style_Fill::FILL_PATTERN_DARKHORIZONTAL;
			case 0x06: return _Style_Fill::FILL_PATTERN_DARKVERTICAL;
			case 0x07: return _Style_Fill::FILL_PATTERN_DARKDOWN;
			case 0x08: return _Style_Fill::FILL_PATTERN_DARKUP;
			case 0x09: return _Style_Fill::FILL_PATTERN_DARKGRID;
			case 0x0A: return _Style_Fill::FILL_PATTERN_DARKTRELLIS;
			case 0x0B: return _Style_Fill::FILL_PATTERN_LIGHTHORIZONTAL;
			case 0x0C: return _Style_Fill::FILL_PATTERN_LIGHTVERTICAL;
			case 0x0D: return _Style_Fill::FILL_PATTERN_LIGHTDOWN;
			case 0x0E: return _Style_Fill::FILL_PATTERN_LIGHTUP;
			case 0x0F: return _Style_Fill::FILL_PATTERN_LIGHTGRID;
			case 0x10: return _Style_Fill::FILL_PATTERN_LIGHTTRELLIS;
			case 0x11: return _Style_Fill::FILL_PATTERN_GRAY125;
			case 0x12: return _Style_Fill::FILL_PATTERN_GRAY0625;
			default:   return _Style_Fill::FILL_NONE;
		}
	}
