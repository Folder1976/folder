...
/**
 * _Reader_Excel2003XML
 *
 * @category   
 * @package    _Reader
 * @copyright  Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Reader_Excel2003XML extends _Reader_Abstract implements _Reader_IReader
{
	/**
	 * Formats
	 *
	...
	/**
	 * Create a new _Reader_Excel2003XML
	 */
...
//		echo 'Character Set is ',$this->_charSet,'<br />';
	...
	public function listWorksheetNames($pFilename)
	{
		...
		if (!$this->canRead($pFilename)) {
			throw new _Reader_Exception($pFilename . " is an Invalid Spreadsheet file.");
		}
	...
	public function loadIntoExisting($pFilename,  $obj)
	{
		...
		$underlineStyles = array (
				_Style_Font::UNDERLINE_NONE,
				_Style_Font::UNDERLINE_DOUBLE,
				_Style_Font::UNDERLINE_DOUBLEACCOUNTING,
				_Style_Font::UNDERLINE_SINGLE,
				_Style_Font::UNDERLINE_SINGLEACCOUNTING
			);
		$verticalAlignmentStyles = array (
				_Style_Alignment::VERTICAL_BOTTOM,
				_Style_Alignment::VERTICAL_TOP,
				_Style_Alignment::VERTICAL_CENTER,
				_Style_Alignment::VERTICAL_JUSTIFY
			);
		$horizontalAlignmentStyles = array (
				_Style_Alignment::HORIZONTAL_GENERAL,
				_Style_Alignment::HORIZONTAL_LEFT,
				_Style_Alignment::HORIZONTAL_RIGHT,
				_Style_Alignment::HORIZONTAL_CENTER,
				_Style_Alignment::HORIZONTAL_CENTER_CONTINUOUS,
				_Style_Alignment::HORIZONTAL_JUSTIFY
			);
		...
		if (isset($xml->CustomDocumentProperties)) {
			foreach($xml->CustomDocumentProperties[0] as $propertyName => $propertyValue) {
				...
				$propertyName = preg_replace_callback('/_x([0-9a-z]{4})_/','_Reader_Excel2003XML::_hex2str',$propertyName);
				$propertyType = _DocumentProperties::PROPERTY_TYPE_UNKNOWN;
				switch((string) $propertyAttributes) {
					case 'string' :
						$propertyType = _DocumentProperties::PROPERTY_TYPE_STRING;
						$propertyValue = trim($propertyValue);
						break;
					case 'boolean' :
						$propertyType = _DocumentProperties::PROPERTY_TYPE_BOOLEAN;
						$propertyValue = (bool) $propertyValue;
						break;
					case 'integer' :
						$propertyType = _DocumentProperties::PROPERTY_TYPE_INTEGER;
						$propertyValue = intval($propertyValue);
						break;
					case 'float' :
						$propertyType = _DocumentProperties::PROPERTY_TYPE_FLOAT;
						$propertyValue = floatval($propertyValue);
						break;
					case 'dateTime.tz' :
						$propertyType = _DocumentProperties::PROPERTY_TYPE_DATE;
...
//									echo $borderStyleKey.' = '.$borderStyleValue.'<br />';
									switch ($borderStyleKey) {
										case 'LineStyle' :
												$thisBorder['style'] = _Style_Border::BORDER_MEDIUM;
			...
			if (isset($worksheet->Table->Column)) {
				foreach($worksheet->Table->Column as $columnData) {
					...
					if (isset($columnData_ss['Index'])) {
						$columnID = _Cell::stringFromColumnIndex($columnData_ss['Index']-1);
					}
					if (isset($columnData_ss['Width'])) {
						$columnWidth = $columnData_ss['Width'];
//						echo '<b>Setting column width for '.$columnID.' to '.$columnWidth.'</b><br />';
						$obj->getActiveSheet()->getColumnDimension($columnID)->setWidth($columnWidth / 5.4);
					}
					++$columnID;
				}
			}
...
//					echo '<b>Row '.$rowID.'</b><br />';
					...
					foreach($rowData->Cell as $cell) {
						...
						if (isset($cell_ss['Index'])) {
							$columnID = _Cell::stringFromColumnIndex($cell_ss['Index']-1);
						}
						...
						if ((isset($cell_ss['MergeAcross'])) || (isset($cell_ss['MergeDown']))) {
							...
							if (isset($cell_ss['MergeAcross'])) {
								$columnTo = _Cell::stringFromColumnIndex(_Cell::columnIndexFromString($columnID) + $cell_ss['MergeAcross'] -1);
							}
							$rowTo = $rowID;
							...
							$obj->getActiveSheet()->mergeCells($cellRange);
						}
							...
							if (isset($cellData_ss['Type'])) {
								...
								switch ($cellDataType) {
									...
									*/
									...
									case 'DateTime' :
											$type = _Cell_DataType::TYPE_NUMERIC;
											$cellValue = _Shared_Date::PHPToExcel(strtotime($cellValue));
											break;
									case 'Error' :
...
//								echo 'FORMULA<br />';
								...
								$columnNumber = _Cell::columnIndexFromString($columnID);
									...
									foreach($temp as &$value) {
										...
										if ($key = !$key) {
											...
											foreach($cellReferences as $cellReference) {
												...
												$A1CellReference = _Cell::stringFromColumnIndex($columnReference-1).$rowReference;
...
//
							$obj->getActiveSheet()->getCell($columnID.$rowID)->setValueExplicit((($hasCalculatedValue) ? $cellDataFormula : $cellValue),$type);
							if ($hasCalculatedValue) {
//								echo 'Formula result is '.$cellValue.'<br />';
								$obj->getActiveSheet()->getCell($columnID.$rowID)->setCalculatedValue($cellValue);
							}
...
//							echo 'Annotation: ',$annotation,'<br />';
							$obj->getActiveSheet()->getComment( $columnID.$rowID )
...
//								echo '<br />';
								if (!$obj->getActiveSheet()->cellExists($columnID.$rowID)) {
									$obj->getActiveSheet()->getCell($columnID.$rowID)->setValue(NULL);
								}
								$obj->getActiveSheet()->getStyle($cellRange)->applyFromArray($this->_styles[$style]);
							}
						}
						++$columnID;
					}
...
//							echo '<b>Setting row height to '.$rowHeight.'</b><br />';
							$obj->getActiveSheet()->getRowDimension($rowID)->setRowHeight($rowHeight);
						}
					}
	...
	private static function _convertStringEncoding($string,$charset) {
		if ($charset != 'UTF-8') {
			return _Shared_String::ConvertEncoding($string,'UTF-8',$charset);
		}
		return $string;
	}
