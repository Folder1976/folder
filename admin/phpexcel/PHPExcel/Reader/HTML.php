 ...
 * @copyright  Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Reader_HTML extends _Reader_Abstract implements _Reader_IReader
{
	/**
	 * Input encoding
	 *
	...
	private $_formats = array( 'h1' => array( 'font' => array( 'bold' => true,
							   ...
							   'a'  => array( 'font' => array( 'underline' => true,
															   'color' => array( 'argb' => _Style_Color::COLOR_BLUE,
															                   ),
															 ),
											),	//	Blue underlined
							   'hr' => array( 'borders' => array( 'bottom' => array( 'style' => _Style_Border::BORDER_THIN,
																					 'color' => array( _Style_Color::COLOR_BLACK,
																					                 ),
																				   ),
																),
											),	//	Bottom border
	...
	//	Data Array used for testing only, should write to  object on completion of tests
...
//						echo 'END OF TABLE ' , $this->_tableLevel , ' CELL<br />';
	...
	public function loadIntoExisting($pFilename,  $obj)
	{
		...
		if (!$this->_isValidFormat()) {
			...
			throw new _Reader_Exception($pFilename . " is an Invalid HTML file.");
		}
		...
		if ($loaded === FALSE) {
			throw new _Reader_Exception('Failed to load ',$pFilename,' as a DOM Document');
		}
		...
		$column = 'A';
		$content = '';
		$this->_processDomElement($dom,$obj->getActiveSheet(),$row,$column,$content);
