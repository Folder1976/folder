...
/**
 * _Worksheet_AutoFilter_Column_Rule
 *
 * @category	
 * @package		_Worksheet
 * @copyright	Copyright (c) 2006 - 2013  (http://www.codeplex.com/)
 */
class _Worksheet_AutoFilter_Column_Rule
{
...
//	const AUTOFILTER_COLUMN_RULE_ALLDATESINQUARTER	= 'allDatesInQuarter';	//	<dynamicFilter type="Q2"/> for Quarter 2
	 ...
	 * Autofilter Column
	 *
	 * @var _Worksheet_AutoFilter_Column
	 */
	...
	/**
	 * Create a new _Worksheet_AutoFilter_Column_Rule
	 *
	 * @param _Worksheet_AutoFilter_Column $pParent
	 */
	public function __construct(_Worksheet_AutoFilter_Column $pParent = NULL)
	{
	...
	public function setRuleType($pRuleType = self::AUTOFILTER_RULETYPE_FILTER) {
		if (!in_array($pRuleType,self::$_ruleTypes)) {
			throw new _Exception('Invalid rule type for column AutoFilter Rule.');
		}
	...
	public function setValue($pValue = '') {
		if (is_array($pValue)) {
			...
			if (count($pValue) == 0) {
				throw new _Exception('Invalid rule value for column AutoFilter Rule.');
			}
	...
	public function setOperator($pOperator = self::AUTOFILTER_COLUMN_RULE_EQUAL) {
		...
		if ((!in_array($pOperator,self::$_operators)) &&
			...
			throw new _Exception('Invalid operator for column AutoFilter Rule.');
		}
	 ...
	 *
	 * @param _Worksheet_AutoFilter_Column
	 * @return _Worksheet_AutoFilter_Column_Rule
	 */
	public function setParent(_Worksheet_AutoFilter_Column $pParent = NULL) {
		$this->_parent = $pParent;
