...
class _Worksheet_PageSetup
{
	 ...
	 * @var int
	 */
	private $_paperSize		= _Worksheet_PageSetup::PAPERSIZE_LETTER;
	 ...
	 * @var string
	 */
	private $_orientation	= _Worksheet_PageSetup::ORIENTATION_DEFAULT;
    ...
    /**
     * Create a new _Worksheet_PageSetup
     */
	...
	public function setScale($pValue = 100, $pUpdate = true) {
		...
		if (($pValue >= 0) || is_null($pValue)) {
			...
			throw new _Exception("Scale must not be negative");
		}
		return $this;
	}
	...
	public function getPrintArea($index = 0) {
		...
		throw new _Exception("Requested Print Area does not exist");
	}
	...
	public function setPrintArea($value, $index = 0, $method = self::SETPRINTRANGE_OVERWRITE) {
		if (strpos($value,'!') !== false) {
			throw new _Exception('Cell coordinate must not specify a worksheet.');
		} elseif (strpos($value,':') === false) {
			throw new _Exception('Cell coordinate must be a range of cells.');
		} elseif (strpos($value,'$') !== false) {
		...
		if ($method == self::SETPRINTRANGE_OVERWRITE) {
			if ($index == 0) {
				...
				if (($index <= 0) || ($index > count($printAreas))) {
		    		throw new _Exception('Invalid index for setting print range.');
				}
    		...
    		throw new _Exception('Invalid method for setting print range.');
		}
    ...
    {
    	return $this->setPrintArea(_Cell::stringFromColumnIndex($column1) . $row1 . ':' . _Cell::stringFromColumnIndex($column2) . $row2, $index, $method);
    }
    ...
    {
    	return $this->setPrintArea(_Cell::stringFromColumnIndex($column1) . $row1 . ':' . _Cell::stringFromColumnIndex($column2) . $row2, $index, self::SETPRINTRANGE_INSERT);
	}
