...
class _Shared_Font
{
	 ...
	 *
	 * @param 	_Style_Font			$font			Font object
	 * @param 	_RichText|string	$cellText		Text to calculate width
	 * @param 	integer						$rotation		Rotation angle
	 * @param 	_Style_Font|NULL	$defaultFont	Font object
	 * @return 	integer		Column width
	 */
	public static function calculateColumnWidth(_Style_Font $font, $cellText = '', $rotation = 0, _Style_Font $defaultFont = null) {

		// If it is rich text, use plain text
		if ($cellText instanceof _RichText) {
		...
		try {
			...
			if (self::$autoSizeMethod == self::AUTOSIZE_METHOD_APPROX) {
				throw new _Exception('AutoSize method is set to approx');
			}
		...
		$columnWidth = _Shared_Drawing::pixelsToCellDimension($columnWidth, $defaultFont);
	 ...
	 * @throws _Exception
	 */
	public static function getTextWidthPixelsExact($text, _Style_Font $font, $rotation = 0) {
		if (!function_exists('imagettfbbox')) {
			throw new _Exception('GD library needs to be enabled');
		}
	 ...
	 */
	public static function getTextWidthPixelsApprox($columnText, _Style_Font $font = null, $rotation = 0)
	{
		...
		switch ($fontName) {
			case 'Calibri':
				...
				$columnWidth = (int) (8.26 * _Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 11; // extrapolate from font size
				break;

			case 'Arial':
				// value 7 was found via interpolation by inspecting real Excel files with Arial 10 font.
				$columnWidth = (int) (7 * _Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 10; // extrapolate from font size
				break;

			case 'Verdana':
				// value 8 was found via interpolation by inspecting real Excel files with Verdana 10 font.
				$columnWidth = (int) (8 * _Shared_String::CountCharacters($columnText));
				$columnWidth = $columnWidth * $fontSize / 10; // extrapolate from font size
				break;
	...
	public static function getTrueTypeFontFileFromFont($font) {
		if (!file_exists(self::$trueTypeFontPath) || !is_dir(self::$trueTypeFontPath)) {
			throw new _Exception('Valid directory to TrueType Font files not specified');
		}
		...
		switch ($name) {
			...
			default:
				throw new _Exception('Unknown font name "'. $name .'". Cannot map to TrueType font file');
				break;
		}
		...
		if (!file_exists($fontFile)) {
			throw New _Exception('TrueType Font file not found');
		}
	 ...
	 *
	 * @param _Style_Font $font The workbooks default font
	 * @param boolean $pPixels true = return column width in pixels, false = return in OOXML units
	 * @return mixed Column width
	 */
	public static function getDefaultColumnWidthByFont(_Style_Font $font, $pPixels = false)
	{
	 ...
	 */
	public static function getDefaultRowHeightByFont(_Style_Font $font)
	{
		switch ($font->getName()) {
			case 'Arial':
				switch ($font->getSize()) {
