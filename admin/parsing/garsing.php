...
?>
<br><br>
<a href="main.php?func=add_products&supplier=garsing&resetlinks">Обнулить сайтмап (заново)</a>&nbsp;&nbsp;|&nbsp;&nbsp;
<a href="main.php?func=add_products&supplier=garsing&links"><b><font color="blue">Продолжить парсить</font></b></a>&nbsp;&nbsp;|&nbsp;&nbsp;
<a href="main.php?func=add_products&supplier=garsing&links&unset"><b><font color="orange">Пропустить товар</font></b></a>&nbsp;&nbsp;|&nbsp;&nbsp;
<?php

$http = 'http://www.garsing.ru/';
//просто парсим ссылки
if(isset($_GET['resetlinks'])){
	
	$sql = 'UPDATE tbl_parsing_garsing SET view = \'0\' WHERE view = "1";';
	$folder->query($sql) or die('==' . $sql);

}
		$sql = 'DELETE FROM tbl_parsing_garsing WHERE url LIKE \'%media/%\';';
...
if(isset($_GET['links'])){
		...
		$sql = 'SELECT count(id) AS id FROM tbl_parsing_garsing;';
		$tmp = $folder->query($sql) or die('==' . $sql);
		$tmp = $tmp->fetch_assoc();
		$all = $tmp['id'];

		$sql = 'SELECT count(id) AS id FROM tbl_parsing_garsing WHERE view = \'0\';';
		$tmp = $folder->query($sql) or die('==' . $sql);
		$tmp = $tmp->fetch_assoc();
		$none = $tmp['id'];
echo '<b>Всего ликов - '.$all.'. Пропарсено - '.($all - $none).'. Осталось - '.$none.'.</b>';
	...
	if(isset($_GET['unset'])){
		$sql = 'SELECT id FROM tbl_parsing_garsing WHERE view = \'0\' LIMIT 1;';
		$url = $folder->query($sql) or die('==' . $sql);
		$list = $url->fetch_assoc();
		$sql = 'UPDATE tbl_parsing_garsing SET view = \'2\' WHERE id = "'.$list['id'].'";';
		$url = $folder->query($sql) or die('==' . $sql);
		...
		$sql = 'SELECT * FROM tbl_parsing_garsing WHERE view = \'0\' LIMIT 1;';
		$url = $folder->query($sql) or die('==' . $sql);
...
echo ' <b>Урл ID - '.$list['id'].'. </b>'; 
			...
			if(isset($_GET['url'])){
				...
				if(strpos($list['url'], $http)){
					...
					$sql = 'UPDATE tbl_parsing_garsing SET `url` = \''.$list['url'].'\' WHERE `id` = \''.$list['id'].'\';';
					$folder->query($sql) or die('==' . $sql);
...
//echo 	$list['url']; die();
				...
				if(!$html){
					$sql = 'UPDATE tbl_parsing_garsing SET view = "1" WHERE `url` = \''.$list['url'].'\';';
					$folder->query($sql) or die('==' . $sql);
				...
				//Это товар
				if($html->find('.detailBlockTextInfo',0)){
					//============================================================================================
					$memo = $html->find('.detailBlockTextInfo',0)->innertext();
					$tmp2 = str_get_html($memo);
						$hrefs = $tmp2->find('a');
						//Удаляем линки в описании
					foreach($hrefs as $href){
						
						$str = $href->outertext();
						$meno = str_replace($href, '', $memo);
...
//echo '<br>'.$artkl.'<br>'; 
				...
				foreach($str_tmp as $option){
					...
					if(strpos($href, $http) !== false){
						...
						*/
						$sql = 'SELECT id FROM tbl_parsing_garsing WHERE url = \''.$href.'\';';
						$t = $folder->query($sql) or die('==' . $sql);
						
						if($t->num_rows == 0){
							if(strpos($href, 'catalogue/') !== false){
								$sql = 'INSERT INTO tbl_parsing_garsing SET
											 `url` = \''.$href.'\',
											 `key` = "'.$name.'",
											 `view` = \''.$view.'\',
				...
				$sql = 'UPDATE tbl_parsing_garsing SET view = \'0\',
