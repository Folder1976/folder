...
//Аллегро парсинг
    ...
    if($list != ''){
        ...
        foreach($str_tmp as $option){
            ...
            $html_tmp = str_get_html($option->innertext());
                echo '<br>'.$html_tmp->find('.name', 0)->innertext();
    ...
    foreach($list as $url){
            ...
            $memo = $html->find('.elUnionBlocks #info', 0);
            ...
            if(isset($params['Цвет:']) AND $_POST['color'] == 1){
                    foreach($params['Цвет:'] as $i => $v){
                        ...
                        $color_size[$code]['color_orig'] = $color_orig;
                         ...
                         foreach($params['Расположение:'] as $wid){
                             if(is_array($val['size'])){
                                 foreach($val['size'] as $siz){
                                    ...
                                    $size_tmp[] = $siz . $size[0].'_';
                                    $color_size[$i]['nacen'][$siz . $size[0].'_'] = $nacen;
                                 }
                             }else{
                               ...
                               $size_tmp[] = $size[0].'_';
                               $color_size[$i]['nacen'][$size[0].'_'] = $nacen;
                               
                             }
                             $price_size[$size[0]] = $price1;
                         }
                         $color_size[$i]['size'] = $size_tmp;
                         //$color_size[$i]['storona'][$size_tmp] = $storona;
...
//echo "<pre>";  print_r(var_dump( $color_size )); echo "</pre>";
    ...
    //return true;
            ...
            if(isset($color_values['color_orig'])){
                echo '<hr>1 Цена оригинал === '.  $str_price.' '.$color_values['color_orig'];
            
                if(strpos($color_values['color_orig'], '(+') !== false){
                            $tmp = explode('(+', $color_values['color_orig']);
                            $color = trim($tmp[0]);
                            $str_price = (int)((int)$str_price * ((100+(int)trim($tmp[1], ' )')) / 100));
                }else{
            ...
            if(isset($color_values['color_orig'])){
                echo '<br>2 С учетом цвета === '.  $str_price.' '.$color_values['color_orig'];
            }
                    ...
                    foreach($size_t as $siz){
                        ...
                        $str_price = $tmp_price;
                        if(isset($color_values['nacen'][$siz])){
                            
                            $str_price = (int)($str_price * $color_values['nacen'][$siz]);
        ...
        //header("Content-Type: text/html; charset=UTF-8");
        //echo "<pre>";  print_r(var_dump( $color_values )); echo "</pre>";
        if(isset($color_values['color_orig'])){                        
            echo '<br>3 С учетом стороны === '.$siz.' '.  $str_price.' '.$color_values['color_orig'];          
        }               
                        $str_artkl =  $model;
