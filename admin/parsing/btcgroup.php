...
?>
<br><br>
<a href="main.php?func=add_products&supplier=btcgroup&resetlinks">Обнулить сайтмап (заново)</a>&nbsp;&nbsp;|&nbsp;&nbsp;
<a href="main.php?func=add_products&supplier=btcgroup&links"><b><font color="blue">Продолжить парсить</font></b></a>&nbsp;&nbsp;|&nbsp;&nbsp;
<a href="main.php?func=add_products&supplier=btcgroup&links&unset"><b><font color="orange">Пропустить товар</font></b></a>&nbsp;&nbsp;|&nbsp;&nbsp;
<?php

$http = 'http://safety.btcgroup.ru/';
//просто парсим ссылки
if(isset($_GET['resetlinks'])){
	
	$sql = 'UPDATE tbl_parsing_btcgroup SET view = \'0\' WHERE view = "1";';
	$folder->query($sql) or die('==' . $sql);

}
		$sql = 'DELETE FROM tbl_parsing_btcgroup WHERE url LIKE \'%media/%\';';
...
if(isset($_GET['links'])){
		...
		$sql = 'SELECT count(id) AS id FROM tbl_parsing_btcgroup;';
		$tmp = $folder->query($sql) or die('==' . $sql);
		$tmp = $tmp->fetch_assoc();
		$all = $tmp['id'];

		$sql = 'SELECT count(id) AS id FROM tbl_parsing_btcgroup WHERE view = \'0\';';
		$tmp = $folder->query($sql) or die('==' . $sql);
		$tmp = $tmp->fetch_assoc();
		$none = $tmp['id'];
echo '<b>Всего ликов - '.$all.'. Пропарсено - '.($all - $none).'. Осталось - '.$none.'.</b>';
	...
	if(isset($_GET['unset'])){
		$sql = 'SELECT id FROM tbl_parsing_btcgroup WHERE view = \'0\' LIMIT 1;';
		$url = $folder->query($sql) or die('==' . $sql);
		$list = $url->fetch_assoc();
		$sql = 'UPDATE tbl_parsing_btcgroup SET view = \'2\' WHERE id = "'.$list['id'].'";';
		$url = $folder->query($sql) or die('==' . $sql);
		...
		$sql = 'SELECT * FROM tbl_parsing_btcgroup WHERE view = \'0\' LIMIT 1;';
		$url = $folder->query($sql) or die('==' . $sql);
...
echo ' <b>Урл ID - '.$list['id'].'. </b>'; 
			...
			if(isset($_GET['url'])){
				...
				if(strpos($list['url'], 'http://safety.btcgroup.ru')){
					$list['url'] = str_replace('http://safety.btcgroup.ru','',$list['url']);
					
					$sql = 'UPDATE tbl_parsing_btcgroup SET `url` = \''.$list['url'].'\' WHERE `id` = \''.$list['id'].'\';';
					$folder->query($sql) or die('==' . $sql);
...
//echo '<br>'.$artkl.'<br>'; 
				...
				foreach($str_tmp as $option){
					...
					if(strpos($href, 'safety.btcgroup.ru/') !== false){
						...
						*/
						$sql = 'SELECT id FROM tbl_parsing_btcgroup WHERE url = \''.$href.'\';';
						$t = $folder->query($sql) or die('==' . $sql);
						
						if($t->num_rows == 0){
							if(strpos($href, 'media/') === false){
								$sql = 'INSERT INTO tbl_parsing_btcgroup SET
											 `url` = \''.$href.'\',
											 `key` = "'.$name.'",
											 `view` = \''.$view.'\',
				...
				$sql = 'UPDATE tbl_parsing_btcgroup SET view = \'0\',
