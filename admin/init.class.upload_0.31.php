                ...
                if ($this->mime_fileinfo) {
                    ...
                    if (function_exists('finfo_open')) {
                        ...
                        if (is_resource($f)) {
                            ...
                            if (preg_match('/^([-\w\s])+$/i', $this->file_src_mime)) {
                                $this->file_src_mime = preg_replace('/^([-\w\s])+$/i', '$1/$2', $this->file_src_mime);
                    ...
                    if (substr(PHP_OS, 0, 3) != 'WIN') {
                        if (function_exists('exec')) {
                            if (strlen($mime = @exec("file -bi ".escapeshellarg($this->file_src_pathname))) != 0) {
                                ...
                                //echo '$this->file_src_mime - '.$this->file_src_mime;
                                if (preg_match('/^([-a-z0-9_\s])+$/i', $this->file_src_mime)) {
                                    $this->file_src_mime = preg_replace('/^([-\w\s])+$/i', '$1/$2', $this->file_src_mime);
                                    $this->log .= '-&nbsp;MIME validated as ' . $this->file_src_mime . '<br />';
                                } else {
                                    $this->file_src_mime = null;
